...
class ModuleTester:
    ...
    def assert_array_equal(self, x, y, err_msg=''):
        ...
        self.assert_array_compare(
            self.equal, x, y, err_msg=err_msg,
    ...
    def test_4(self):
        ...
        x = x.reshape(2, 3, 4)
        y = y.reshape(2, 3, 4)
        assert self.allequal(np.transpose(y, (2, 0, 1)),
                             self.transpose(x, (2, 0, 1)))
        assert self.allequal(np.take(y, (2, 0, 1), 1),
                             self.take(x, (2, 0, 1), 1))
        assert self.allequal(
            np.inner(self.filled(x, 0), self.filled(y, 0)),
            self.inner(x, y),
        )
        assert self.allequal(
            np.outer(self.filled(x, 0), self.filled(y, 0)),
            self.outer(x, y),
        )
    ...
    def test_99(self):
        ...
        self.assert_array_equal(2.0, self.average(
            ott, weights=[1., 1., 2., 1.]))
        ...
        self.assert_array_equal(self.average(
            y, None), np.add.reduce(np.arange(6))*3./12.)
        ...
        m5 = [0, 1, 1, 1, 1, 1]
        self.assert_array_equal(self.average(
            self.masked_array(x, m1), axis=0), 2.5)
        self.assert_array_equal(self.average(
            self.masked_array(x, m2), axis=0), 2.5)
        self.assert_array_equal(self.average(
            self.masked_array(x, m5), axis=0), 0.0)
        self.assert_array_equal(self.count(
            self.average(self.masked_array(x, m4), axis=0)), 0)
        z = self.masked_array(y, m3)
        self.assert_array_equal(self.average(z, None), 20./6.)
        self.assert_array_equal(self.average(z, axis=0), [
                                0., 1., 99., 99., 4.0, 7.5])
