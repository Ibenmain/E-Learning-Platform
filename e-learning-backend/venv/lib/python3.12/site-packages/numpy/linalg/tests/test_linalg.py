...
def consistent_subclass(out, in_):
    ...
    return type(out) is (
        type(in_) if isinstance(in_, np.ndarray)
        else np.ndarray
    )
...
def get_complex_dtype(dtype):
    return {
        single: csingle, double: cdouble,
        csingle: csingle, cdouble: cdouble,
    }[dtype]
...
CASES += apply_tag(
    'square', [
        ...
        LinalgCase(
            'cdouble_2',
            array([[1. + 2j, 2 + 3j], [3 + 4j, 4 + 5j]], dtype=cdouble),
            array([[2. + 1j, 1. + 2j, 1 + 3j],
                  [1 - 2j, 1 - 3j, 1 - 6j]], dtype=cdouble),
        ),
        LinalgCase(
            '0x0',
...
CASES += apply_tag(
    'nonsquare', [
        ...
        LinalgCase(
            ...
            array([[2. + 1j, 1. + 2j], [1 - 1j, 2 - 2j],
                  [1 - 1j, 2 - 2j]], dtype=cdouble),
        ),
        LinalgCase(
            '8x11',
            np.random.rand(8, 11),
    ...
    for case in CASES:
        for a, a_label in _stride_comb_iter(case.a):
            for b, b_label in _stride_comb_iter(case.b):
                new_case = LinalgCase(
                    case.name + '_' + a_label + '_' + b_label, a, b,
...
class _TestNormGeneral(_TestNormBase):
    ...
    def test_vector(self):
        ...
        for v in (
            array(a, dtype=self.dt), array(b, dtype=self.dt),
...
class TestQR:
    ...
    def check_qr_stacked(self, a):
        ...
        assert_almost_equal(
            matmul(swapaxes(q1, -1, -2).conj(), q1),
            stack_I_mat,
        )
        assert_almost_equal(np.triu(r1[..., :, :]), r1)
...
def test_sdot_bug_8577():
    ...
    for bad_lib in bad_libs:
        code = template.format(
            before='import numpy as np', after='',
            bad_lib=bad_lib,
        )
        ...
        code = template.format(
            after='import numpy as np', before='',
            bad_lib=bad_lib,
        )
        subprocess.check_call([sys.executable, '-c', code])
