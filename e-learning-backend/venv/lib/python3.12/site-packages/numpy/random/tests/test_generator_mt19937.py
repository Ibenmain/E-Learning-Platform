...
class TestMultinomial:
    ...
    def test_size(self):
        ...
        assert_equal(random.multinomial(1, p, (2, 2)).shape, (2, 2, 2))
        assert_equal(
            random.multinomial(1, p, np.array((2, 2))).shape,
            (2, 2, 2),
        )

        assert_raises(
            TypeError, random.multinomial, 1, p,
            float(1),
        )
    ...
    def test_multinomial_pvals_float32(self):
        x = np.array(
            [
                9.9e-01, 9.9e-01, 1.0e-09, 1.0e-09, 1.0e-09, 1.0e-09,
...
class TestIntegers:
    ...
    itype = [
        bool, np.int8, np.uint8, np.int16, np.uint16,
    ...
    def test_bounds_checking(self, endpoint):
        for dt in self.itype:
            ...
            assert_raises(
                ValueError, self.rfunc, lbnd - 1, ubnd,
                endpoint=endpoint, dtype=dt,
            )
            assert_raises(
                ValueError, self.rfunc, lbnd, ubnd + 1,
                endpoint=endpoint, dtype=dt,
            )
            assert_raises(
                ValueError, self.rfunc, ubnd, lbnd,
                endpoint=endpoint, dtype=dt,
            )
            assert_raises(
                ValueError, self.rfunc, 1, 0, endpoint=endpoint,
                dtype=dt,
            )
            ...
            assert_raises(
                ValueError, self.rfunc, 1, [0],
                endpoint=endpoint, dtype=dt,
            )
            assert_raises(
                ValueError, self.rfunc, [ubnd+1], [ubnd],
                endpoint=endpoint, dtype=dt,
            )
    ...
    def test_bounds_checking_array(self, endpoint):
        for dt in self.itype:
            ...
            assert_raises(
                ValueError, self.rfunc, [lbnd - 1] * 2, [ubnd] * 2,
                endpoint=endpoint, dtype=dt,
            )
            assert_raises(
                ValueError, self.rfunc, [lbnd] * 2,
                [ubnd + 1] * 2, endpoint=endpoint, dtype=dt,
            )
            assert_raises(
                ValueError, self.rfunc, ubnd, [lbnd] * 2,
                endpoint=endpoint, dtype=dt,
            )
            assert_raises(
                ValueError, self.rfunc, [1] * 2, 0,
                endpoint=endpoint, dtype=dt,
            )
    ...
    def test_in_bounds_fuzz(self, endpoint):
        ...
        vals = self.rfunc(
            0, 2 - endpoint, size=2 ** 16, endpoint=endpoint,
            dtype=bool,
        )
        assert_(vals.max() < 2)
        assert_(vals.min() >= 0)
    ...
    def test_scalar_array_equiv(self, endpoint):
        for dt in self.itype:
            ...
            scalar = random.integers(
                lbnd, ubnd, size=size, endpoint=endpoint,
                dtype=dt,
            )
    ...
    def test_repeatability(self, endpoint):
        ...
        for dt in self.itype[1:]:
            ...
            if sys.byteorder == 'little':
                val = random.integers(
                    0, 6 - endpoint, size=1000, endpoint=endpoint,
                    dtype=dt,
                )
            else:
                val = random.integers(
        ...
        random = Generator(MT19937(1234))
        val = random.integers(
            0, 2 - endpoint, size=1000, endpoint=endpoint,
            dtype=bool,
        ).view(np.int8)
        res = hashlib.sha256(val).hexdigest()
    ...
    def test_repeatability_broadcasting(self, endpoint):
        for dt in self.itype:
            ...
            random = Generator(MT19937(1234))
            val = random.integers(
                lbnd, ubnd, size=1000, endpoint=endpoint,
                dtype=dt,
            )

            random = Generator(MT19937(1234))
            val_bc = random.integers(
                [lbnd] * 1000, ubnd, endpoint=endpoint,
                dtype=dt,
            )
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                2**32 - 1, np.array([
                    517043486, 1364798665, 1733884389, 1353720612,
                    3769704066, 1170797179, 4108474671,
                ]),
            ),
            (
                2**32, np.array([
                    517043487, 1364798666, 1733884390, 1353720613,
                    3769704067, 1170797180, 4108474672,
                ]),
            ),
            (
                2**32 + 1, np.array([
                    517043487, 1733884390, 3769704068, 4108474673,
    ...
    def test_int64_uint64_broadcast_exceptions(self, endpoint):
        ...
        for dtype in configs:
            for config in configs[dtype]:
                ...
                assert_raises(
                    ValueError, random.integers, low, high,
                    endpoint=endpoint, dtype=dtype,
                )
                assert_raises(
                    ValueError, random.integers, low_a, high,
                    endpoint=endpoint, dtype=dtype,
                )
                assert_raises(
                    ValueError, random.integers, low, high_a,
                    endpoint=endpoint, dtype=dtype,
                )
                assert_raises(
                    ValueError, random.integers, low_a, high_a,
                    endpoint=endpoint, dtype=dtype,
                )
                ...
                assert_raises(
                    ValueError, random.integers, low_o, high,
                    endpoint=endpoint, dtype=dtype,
                )
                assert_raises(
                    ValueError, random.integers, low, high_o,
                    endpoint=endpoint, dtype=dtype,
                )
                assert_raises(
                    ValueError, random.integers, low_o, high_o,
                    endpoint=endpoint, dtype=dtype,
                )
    ...
    def test_respect_dtype_array(self, endpoint):
        ...
        for dt in self.itype:
            ...
            assert_equal(sample.dtype, dt)
            sample = self.rfunc(
                [lbnd] * 2, [ubnd] * 2, endpoint=endpoint,
                dtype=dt,
            )
            assert_equal(sample.dtype, dt)
    ...
    def test_zero_size(self, endpoint):
        ...
        for dt in self.itype:
            ...
            assert self.rfunc(
                0, -10, 0, endpoint=endpoint,
                dtype=dt,
            ).shape == (0,)
            assert_equal(
                random.integers(0, 0, size=(3, 0, 4)).shape,
                (3, 0, 4),
            )
...
class TestRandomDist:
    ...
    def test_choice_return_shape(self):
        ...
        assert_equal(
            random.choice(['a', 'b'], size=(3, 0, 4)).shape,
            (3, 0, 4),
        )
    ...
    def test_multivariate_normal(self, method):
        ...
        assert_raises(
            LinAlgError, random.multivariate_normal, mean, cov,
            method='cholesky',
        )
        ...
        assert_raises(
            ValueError, random.multivariate_normal,
            np.zeros((2, 1, 1)), cov,
        )
        assert_raises(
            ValueError, random.multivariate_normal,
    ...
    def test_negative_binomial_exceptions(self):
        with np.errstate(invalid='ignore'):
            ...
            assert_raises(
                ValueError, random.negative_binomial, 100,
                [np.nan] * 10,
            )
    ...
    def test_noncentral_f(self):
        random = Generator(MT19937(self.seed))
        actual = random.noncentral_f(
            dfnum=5, dfden=2, nonc=1,
            size=(3, 2),
        )
        desired = np.array([
    ...
    def test_pareto(self):
        ...
        desired = np.array([
            [1.0394926776069018e+00, 7.7142534343505773e+04],
    ...
    def test_standard_gamma_unknown_type(self):
        assert_raises(
            TypeError, random.standard_gamma, 1.,
            dtype='int32',
        )
    ...
    def test_out_size_mismatch(self):
        out = np.zeros(10)
        assert_raises(
            ValueError, random.standard_gamma, 10.0, size=20,
            out=out,
        )
        assert_raises(
            ValueError, random.standard_gamma, 10.0, size=(10, 1),
            out=out,
        )
    ...
    def test_triangular(self):
        ...
        actual = random.triangular(
            left=5.12, mode=10.23, right=20.34,
            size=(3, 2),
        )
        desired = np.array([
    ...
    def test_scalar_exception_propagation(self):
        ...
        assert_raises(
            TypeError, random.uniform, throwing_float,
            throwing_float,
        )
...
class TestBroadcast:
    ...
    def test_triangular(self):
        ...
        assert_raises(
            ValueError, triangular, bad_left_two, bad_mode_two * 3,
            right,
        )
        ...
        assert_raises(
            ValueError, triangular, bad_left_two, bad_mode_two,
            right * 3,
        )
    ...
    def test_hypergeometric(self):
        ...
        assert_array_equal(actual, desired)
        assert_raises(ValueError, random.hypergeometric,
                      bad_ngood * 3, nbad, nsample)
        assert_raises(ValueError, random.hypergeometric,
                      ngood * 3, bad_nbad, nsample)
        assert_raises(ValueError, random.hypergeometric,
                      ngood * 3, nbad, bad_nsample_one)
        assert_raises(ValueError, random.hypergeometric,
                      ngood * 3, nbad, bad_nsample_two)

        random = Generator(MT19937(self.seed))
        actual = random.hypergeometric(ngood, nbad * 3, nsample)
        assert_array_equal(actual, desired)
        assert_raises(ValueError, random.hypergeometric,
                      bad_ngood, nbad * 3, nsample)
        assert_raises(ValueError, random.hypergeometric,
                      ngood, bad_nbad * 3, nsample)
        assert_raises(ValueError, random.hypergeometric,
                      ngood, nbad * 3, bad_nsample_one)
        assert_raises(ValueError, random.hypergeometric,
                      ngood, nbad * 3, bad_nsample_two)
...
class TestSingleEltArrayInput:
    ...
    def test_one_arg_funcs(self):
        funcs = (
            random.exponential, random.standard_gamma,
    ...
    def test_three_arg_funcs(self):
        funcs = [
            random.noncentral_f, random.triangular,
