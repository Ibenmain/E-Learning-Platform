...
def comp_state(state1, state2):
    ...
    if isinstance(state1, dict):
        ...
        if (
            isinstance(state1, (list, tuple, np.ndarray)) and isinstance(
...
class RNG:
    ...
    def test_standard_exponential_float_log(self):
        ...
        params_0(
            partial(
                self.rg.standard_exponential, dtype='float32',
                method='inv',
            ),
        )
    ...
    def test_integers_broadcast(self, dtype):
        ...
        d = self.rg.integers(
            np.array(
                [lower] * 10,
            ), np.array([upper], dtype=object), size=10,
            dtype=dtype,
        )
        assert_equal(a, d)
        self._reset_state()
