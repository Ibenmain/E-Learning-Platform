...
f2cmap_all = {
    ...
    'integer': {
        '': 'int', '1': 'signed_char', '2': 'short',
        '4': 'int', '8': 'long_long',
        '-1': 'unsigned_char', '-2': 'unsigned_short',
        '-4': 'unsigned', '-8': 'unsigned_long_long',
    },
    'complex': {
        '': 'complex_float', '8': 'complex_float',
        '16': 'complex_double', '24': 'complex_long_double',
        '32': 'complex_long_double',
    },
    'complexkind': {
        '': 'complex_float', '4': 'complex_float',
        '8': 'complex_double', '12': 'complex_long_double',
        '16': 'complex_long_double',
    },
    'logical': {
        '': 'int', '1': 'char', '2': 'short', '4': 'int',
        '8': 'long_long',
    },
    ...
    if isstring(var) and not isarray(var):
        ...
        for d in var['dimension']:
            ...
            if d not in ['*', ':', '(*)', '(:)']:
                ...
                errmess(
                    'getarrdims: If in call-back function: array argument {} must have bounded dimensions: got {}\n'.format(
                        repr(a), repr(d)),
                )
        if ret['cbsetdims']:
...
def getpydocsign(a, var):
    ...
    if isscalar(var):
        if isintent_inout(var):
            sig = '{} : {} rank-0 array({},\'{}\'){}'.format(
                a, opt, c2py_map[ctype],
                c2pycode_map[ctype], init,
            )
        else:
            sig = '{} : {} {}{}'.format(a, opt, c2py_map[ctype], init)
        sigout = '{} : {}'.format(out_a, c2py_map[ctype])
    elif isstring(var):
        if isintent_inout(var):
            sig = '{} : {} rank-0 array(string(len={}),\'c\'){}'.format(
                a, opt, getstrlength(var), init,
            )
        else:
            sig = '{} : {} string(len={}){}'.format(
                a, opt, getstrlength(var), init,
            )
        sigout = '{} : string(len={})'.format(out_a, getstrlength(var))
    elif isarray(var):
        dim = var['dimension']
        rank = repr(len(dim))
        ...
        if a in lcb_map and lcb_map[a] in lcb2_map and 'argname' in lcb2_map[lcb_map[a]]:
            ...
            if not ua == a:
                ...
                ua = ''
        sig = '{} : call-back function{}'.format(a, ua)
        sigout = sig
    else:
        errmess(
    ...
    if hasinitvalue(var):
        ...
        if iscomplex(var) or iscomplexarray(var):
            ...
            try:
                ...
                raise ValueError(
                    'getinit: expected complex number `(r,i)\' but got `{}\' as initial value of {!r}.'.format(
                        init, a),
                )
            if isarray(var):
...
def sign2map(a, var):
    ...
    if isexternal(var):
        ...
        if a in lcb_map:
            ...
            ret['cbname'] = a
            errmess(
                'sign2map: Confused: external {} is not in lcb_map{}.\n'.format(
                    a, list(lcb_map.keys()),
                ),
            )
    if isstring(var):
    ...
    if debugcapi(var):
        il = [
            isintent_in, 'input', isintent_out, 'output',
        ...
        if isarray(var):
            ddim = ','.join(
                map(lambda x, y: '{}|{}'.format(x, y), var['dimension'], dim),
            )
    ...
    if ret['ctype'] in cformat_map:
        ret['varshowvalue'] = '#name#:{}={}'.format(
            a, cformat_map[ret['ctype']])
        ret['showvalueformat'] = '%s' % (cformat_map[ret['ctype']])
    if isstring(var):
        ret['varshowvalue'] = '#name#:slen({})=%d {}=\\"%s\\"'.format(a, a)
...
def routsign2map(rout):
    ...
    if 'use' in rout:
        ...
        errmess(
            'routsign2map: Confused: function {} has externals {} but no "use" statement.\n'.format(
...
def cb_routsign2map(rout, um):
    ...
    """
    ret = {
        'name': 'cb_{}_in_{}'.format(rout['name'], um),
        'returncptr': '',
    }
