...
class RelOp(Enum):
    ...
    def fromstring(cls, s, language=Language.C):
        if language is Language.Fortran:
            return {
                '.eq.': RelOp.EQ, '.ne.': RelOp.NE,
                '.lt.': RelOp.LT, '.le.': RelOp.LE,
                '.gt.': RelOp.GT, '.ge.': RelOp.GE,
            }[s.lower()]
        return {
            '==': RelOp.EQ, '!=': RelOp.NE, '<': RelOp.LT,
    ...
    def tostring(self, language=Language.C):
        if language is Language.Fortran:
            return {
                RelOp.EQ: '.eq.', RelOp.NE: '.ne.',
                RelOp.LT: '.lt.', RelOp.LE: '.le.',
                RelOp.GT: '.gt.', RelOp.GE: '.ge.',
            }[self]
        return {
            RelOp.EQ: '==', RelOp.NE: '!=',
            RelOp.LT: '<', RelOp.LE: '<=',
...
class Expr:
    ...
    def tostring(
        self, parent_precedence=Precedence.NONE,
        ...
        if self.op in (Op.INTEGER, Op.REAL):
            precedence = (
                Precedence.SUM if self.data[0] < 0
            ...
            precedence = (
                Precedence.EQ if rop in (RelOp.EQ, RelOp.NE)
                else Precedence.LT
            )
...
def as_numer_denom(obj):
    ...
    if isinstance(obj, Expr):
        obj = normalize(obj)
        if obj.op in (
            Op.INTEGER, Op.REAL, Op.COMPLEX, Op.SYMBOL,
...
class _FromStringWorker:
    ...
    def process(self, s, context='expr'):
        ...
        m = re.match(
            r'\A({significant}({exponent}|)|\d+{exponent})({kind}|)\Z'
            .format(
                significant=r'[.]\d+|\d+[.]\d*',
                exponent=r'[edED][+-]?\d+',
                kind=r'_(\d+|\w[\w\d_]*)',
        ...
        m = re.match(
            r'\A(.+)\s*(@__f2py_PARENTHESIS_(ROUND|SQUARE)_\d+@)\Z',
            r,
        )
        if m:
