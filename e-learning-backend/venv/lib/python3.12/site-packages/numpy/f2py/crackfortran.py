...
def readfortrancode(ffile, dowithline=show, istop=1):
    ...
    while True:
        ...
        if sourcecodeform == 'fix':
            ...
            raise ValueError(
                "Flag sourcecodeform must be either 'fix' or 'free': %s" % repr(
                    sourcecodeform),
            )
        ...
        if m:
            ...
            if os.path.isfile(fn):
                ...
                if not foundfile:
                    outmess(
                        'readfortrancode: could not find include file {} in {}. Ignoring.\n'.format(
...
typespattern = re.compile(
    beforethisafter % ('', fortrantypes, fortrantypes, '.*'), re.I,
), 'type'
...
beginpattern77 = re.compile(
    beforethisafter % ('', groupbegins77, groupbegins77, '.*'), re.I,
), 'begin'
groupbegins90 = groupbegins77 + \
    r'|module(?!\s*procedure)|python\s*module|(abstract|)\s*interface|' + \
    r'type(?!\s*\()'
beginpattern90 = re.compile(
    beforethisafter % ('', groupbegins90, groupbegins90, '.*'), re.I,
), 'begin'
groupends = (
...
implicitpattern = re.compile(
    beforethisafter % ('', 'implicit', 'implicit', '.*'), re.I,
), 'implicit'
dimensionpattern = re.compile(
    beforethisafter % (
        '', 'dimension|virtual', 'dimension|virtual', '.*',
    ), re.I,
), 'dimension'
externalpattern = re.compile(
    beforethisafter % ('', 'external', 'external', '.*'), re.I,
), 'external'
optionalpattern = re.compile(
    beforethisafter % ('', 'optional', 'optional', '.*'), re.I,
), 'optional'
requiredpattern = re.compile(
    beforethisafter % ('', 'required', 'required', '.*'), re.I,
), 'required'
publicpattern = re.compile(
    beforethisafter % ('', 'public', 'public', '.*'), re.I,
), 'public'
privatepattern = re.compile(
    beforethisafter % ('', 'private', 'private', '.*'), re.I,
), 'private'
intrinsicpattern = re.compile(
    beforethisafter % ('', 'intrinsic', 'intrinsic', '.*'), re.I,
), 'intrinsic'
intentpattern = re.compile(
    beforethisafter % (
...
multilinepattern = re.compile(
    r"\s*(?P<before>''')(?P<this>.*?)(?P<after>''')\s*\Z", re.S,
), 'multiline'
##
...
def crackline(line, reset=0):
    ...
    if has_semicolon and not (
        f2pyenhancementspattern[0].match(line) or
    ...
    flag = 0
    for pat in [
        dimensionpattern, externalpattern, intentpattern, optionalpattern,
        requiredpattern,
        ...
        if 'externals' in groupcache[groupcounter]:
            for name in groupcache[groupcounter]['externals']:
                ...
                if 'interfaced' in groupcache[groupcounter] and name in groupcache[groupcounter]['interfaced']:
                    continue
                m1 = re.match(
                    r'(?P<before>[^"]*)\b%s\b\s*@\(@(?P<args>[^@]*)@\)@.*\Z' % name, markouterparen(
                        line), re.I,
                )
                if m1:
                    ...
                    if m2:
                        line = 'callfun {}({}) result ({})'.format(
                            name, a, m2.group('result'),
                        )
                    else:
                        line = 'callfun {}({})'.format(name, a)
...
def appenddecl(decl, decl2, force=1):
    ...
    for k in list(decl2.keys()):
        if k == 'typespec':
            ...
            'intent', 'check', 'dimension', 'optional',
            'required', 'depend',
        ]:
            errmess('appenddecl: "%s" not implemented.\n' % k)
        else:
            raise Exception(
                'appenddecl: Unknown variable definition key: ' +
                str(k),
            )
    return decl
...
def analyzeline(m, case, line):
    ...
    if case in ['begin', 'call', 'callfun']:
        ...
        if needinterface:
            ...
            groupcache[groupcounter]['from'] = '{}:{}'.format(
                groupcache[groupcounter -
                           1]['from'], groupcache[groupcounter - 1]['name'],
            )
            groupcache[groupcounter]['body'] = []
            groupcache[groupcounter]['externals'] = []
            groupcache[groupcounter]['interfaced'] = []
        ...
        if case in ['call', 'callfun']:  # set parents variables
            ...
            try:
                del groupcache[groupcounter]['vars'][name][
                    groupcache[groupcounter]['vars'][name]['attrspec'].index(
                        'external')
                ]
            except Exception:
                pass
        ...
        for e in [x.strip() for x in markoutercomma(ll).split('@,@')]:
            ...
            if not m1:
                if case in ['public', 'private']:
                    ...
                    print(m.groupdict())
                    outmess(
                        'analyzeline: no name pattern found in {} statement for {}. Skipping.\n'.format(
                            case, repr(e),
                        ),
                    )
                    continue
        ...
        for e in markoutercomma(ll).split('@,@'):
            try:
                ...
                outmess(
                    'analyzeline: could not extract name,expr in parameter statement "{}" of "{}"\n'.format(
                        e, ll),
                )
                continue
            ...
            if '=' in edecl[k] and (not edecl[k]['='] == initexpr):
                outmess(
                    'analyzeline: Overwriting the value of parameter "{}" ("{}") with "{}".\n'.format(
                        k, edecl[k]['='], initexpr,
                    ),
                )
                ...
                if t.get('typespec') == 'real':
                    ...
                    for m in real16pattern.finditer(initexpr):
                        tt[m.start():m.end()] = list(
                            initexpr[m.start():m.end()].lower().replace(
                                'd', 'e'),
                        )
                    initexpr = ''.join(tt)
        ...
        for l in ll:
            ...
            for idx, v in enumerate(rmbadname([x.strip() for x in markoutercomma(l[0]).split('@,@')])):
                ...
                if current_val and (current_val != new_val):
                    outmess('analyzeline: changing init expression of "{}" ("{}") to "{}"\n'.format(
    ...
    for e in el1:
        ...
        if ename in groupcache[groupcounter]['vars']:
            ...
            if not_has_typespec:
                ...
                outmess(
                    'updatevars: attempt to change the type of "{}" ("{}") to "{}". Ignoring.\n'.format(
            ...
            if 'kindselector' not in edecl:
                ...
                for k in list(kindselect.keys()):
                    if k in edecl['kindselector'] and (not kindselect[k] == edecl['kindselector'][k]):
                        outmess(
                            'updatevars: attempt to change the kindselector "{}" of "{}" ("{}") to "{}". Ignoring.\n'.format(
            ...
            if 'charselector' not in edecl and charselect:
                ...
                for k in list(charselect.keys()):
                    if k in edecl['charselector'] and (not charselect[k] == edecl['charselector'][k]):
                        outmess(
                            'updatevars: attempt to change the charselector "{}" of "{}" ("{}") to "{}". Ignoring.\n'.format(
            ...
            if 'typename' not in edecl:
                ...
                outmess(
                    'updatevars: attempt to change the typename of "{}" ("{}") to "{}". Ignoring.\n'.format(
            ...
            if m1:
                ...
                if 'init' in d1:
                    if '=' in edecl and (not edecl['='] == d1['init']):
                        outmess(
                            'updatevars: attempt to change the init expression of "{}" ("{}") to "{}". Ignoring.\n'.format(
                                ename, edecl['='], d1['init'],
                            ),
                        )
                    else:
...
def cracktypespec(typespec, selector):
    ...
    if selector:
        if typespec in ['complex', 'integer', 'logical', 'real']:
            ...
            outmess(
                'cracktypespec: no selector used for %s\n' %
                (repr(selector)),
            )
...
def postcrack2(block, tab='', param_map=None):
    ...
    if isinstance(block, list):
        ret = [
            postcrack2(g, tab=tab + '\t', param_map=param_map)
            for g in block
        ]
        return ret
    setmesstext(block)
    outmess('{}Block: {}\n'.format(tab, block['name']), 0)
    ...
    new_body = [
        postcrack2(b, tab=tab + '\t', param_map=param_map)
        for b in block['body']
    ]
    block['body'] = new_body
...
def postcrack(block, args=None, tab=''):
    ...
    if not isinstance(block, dict) and 'block' not in block:
        raise Exception(
            'postcrack: Expected block dictionary instead of ' +
            str(block),
        )
    ...
    if block.get('externals'):
        ...
        if interface['vars'] or interface['body']:
            ...
            mblock = {
                'block': 'python module', 'body': [
                    interface,
                ], 'vars': {}, 'name': mname, 'interfaced': block['externals'],
            }
            useblock[mname] = {}
    ...
    for k in list(block['common'].keys()):
        ...
        for e in block['common'][k]:
            ...
            if m:
                ...
                n = e
                errmess(
                    'analyzecommon: failed to extract "<name>[(<dims>)]" from "{}" in common /{}/.\n'.format(
                        e, k),
                )
            comvars.append(n)
        ...
        if 'typespec' not in vars[n]:
            if not ('attrspec' in vars[n] and 'external' in vars[n]['attrspec']):
                if implicitrules:
                    ...
                    outmess(
                        'analyzevars: typespec of variable {} is not defined in routine {}.\n'.format(
                            repr(n), block['name'],
                        ),
                    )
...
def param_eval(v, g_params, params, dimspec=None):
    ...
    if len(dimrange) == 1:
        ...
        raise ValueError(
            'param_eval: multidimensional array parameters '
...
def crack2fortrangen(block, tab='\n', as_interface=False):
    ...
    if 'f2pyenhancements' in block:
        for k in list(block['f2pyenhancements'].keys()):
            f2pyenhancements = '{}{}{} {}'.format(
                f2pyenhancements, tab +
                tabchar, k, block['f2pyenhancements'][k],
            )
...
def common2fortran(common, tab=''):
    ...
    for k in list(common.keys()):
        if k == '_BLNK_':
            ret = '{}{}common {}'.format(ret, tab, ','.join(common[k]))
        else:
            ret = '{}{}common /{}/ {}'.format(ret, tab, k, ','.join(common[k]))
    return ret
        ...
        if 'map' in use[m] and use[m]['map']:
            ...
            for k in list(use[m]['map'].keys()):
                if k == use[m]['map'][k]:
                    ...
                    ret = '{}{}{}=>{}'.format(ret, c, k, use[m]['map'][k])
                    c = ','
        if ret and ret[-1] == ',':
            ret = ret[:-1]
    return ret
...
def vars2fortran(block, vars, args, tab='', as_interface=False):
    ...
    for a in nout:
        if 'depend' in vars[a]:
            for d in vars[a]['depend']:
                if d in vars and 'depend' in vars[d] and a in vars[d]['depend']:
                    errmess(
                        'vars2fortran: Warning: cross-dependence between variables "{}" and "{}"\n'.format(
                            a, d),
                    )
        if 'externals' in block and a in block['externals']:
            if isintent_callback(vars[a]):
                ret = '{}{}intent(callback) {}'.format(ret, tab, a)
            ret = '{}{}external {}'.format(ret, tab, a)
            if isoptional(vars[a]):
                ret = '{}{}optional {}'.format(ret, tab, a)
        ...
        if vardef == 'type' and 'typename' in vars[a]:
            vardef = '{}({})'.format(vardef, vars[a]['typename'])
        selector = {}
        ...
        if '*' in selector:
            if selector['*'] in ['*', ':']:
                vardef = '{}*({})'.format(vardef, selector['*'])
            else:
                vardef = '{}*{}'.format(vardef, selector['*'])
        else:
            if 'len' in selector:
                vardef = '{}(len={}'.format(vardef, selector['len'])
                if 'kind' in selector:
                    vardef = '{},kind={})'.format(vardef, selector['kind'])
                else:
                    vardef = '%s)' % (vardef)
        ...
        if 'attrspec' in vars[a]:
            ...
            if attr:
                vardef = '{}, {}'.format(vardef, ','.join(attr))
                c = ','
        if 'dimension' in vars[a]:
            vardef = '{}{}dimension({})'.format(
        ...
        if 'intent' in vars[a]:
            ...
            if lst:
                vardef = '{}{}intent({})'.format(vardef, c, ','.join(lst))
            c = ','
        if 'check' in vars[a]:
            vardef = '{}{}check({})'.format(
                vardef, c, ','.join(vars[a]['check']))
            c = ','
        if 'depend' in vars[a]:
            vardef = '{}{}depend({})'.format(
        ...
        if '=' in vars[a]:
            ...
            if vars[a]['typespec'] in ['complex', 'double complex']:
                try:
                    v = eval(v)
                    v = '({},{})'.format(v.real, v.imag)
                except Exception:
                    pass
            vardef = '{} :: {}={}'.format(vardef, a, v)
        else:
            vardef = '{} :: {}'.format(vardef, a)
        ret = '{}{}{}'.format(ret, tab, vardef)
    return ret
######
