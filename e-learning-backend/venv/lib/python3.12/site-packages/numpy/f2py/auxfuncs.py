...
def isoptional(var):
    return (
        'attrspec' in var and 'optional' in var['attrspec'] and
...
def isintent_inout(var):
    return (
        'intent' in var and (
            'inout' in var['intent'] or
            'outin' in var['intent']
        ) and 'in' not in var['intent'] and
...
isintent_dict = {
    isintent_in: 'INTENT_IN', isintent_inout: 'INTENT_INOUT',
...
def getargs2(rout):
    ...
    auxvars = [
        a for a in rout['vars'].keys() if isintent_aux(rout['vars'][a])
        and a not in args
    ]
    args = auxvars + args
    if 'sortvars' in rout:
