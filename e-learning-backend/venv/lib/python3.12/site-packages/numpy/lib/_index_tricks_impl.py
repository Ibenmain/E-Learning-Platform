...
from numpy._core import linspace
from numpy._core import overrides
from numpy._core.multiarray import ravel_multi_index
from numpy._core.multiarray import unravel_index
    ...
    def __getitem__(self, key):
        ...
        for k, item in enumerate(key):
            ...
            if isinstance(item, slice):
                ...
                if k != 0:
                    raise ValueError(
                        'special directives must be the '
                        'first entry.',
                    )
                if item in ('r', 'c'):
                    matrix = True
        ...
        if len(result_type_objs) != 0:
            ...
            objs = [
                array(
                    obj, copy=None, subok=True,
                    ndmin=ndmin, dtype=final_dtype,
                ) for obj in objs
            ]
...
class ndindex:
    ...
    def __init__(self, *shape):
        if len(shape) == 1 and isinstance(shape[0], tuple):
            shape = shape[0]
        x = as_strided(
            _nx.zeros(1), shape=shape,
            strides=_nx.zeros_like(shape),
        )
        self._it = _nx.nditer(
            x, flags=['multi_index', 'zerosize_ok'],
            order='C',
        )
