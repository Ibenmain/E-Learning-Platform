...
def _ravel_and_check_weights(a, weights):
    ...
    if a.dtype == np.bool:
        warnings.warn(
            'Converting input from {} to {} for compatibility.'
...
def _unsigned_subtract(a, b):
    ...
    try:
        ...
        return np.subtract(
            np.asarray(a, dtype=dt), np.asarray(b, dtype=dt),
    ...
    *includes* 4.
    ...
    if uniform_bins is not None and simple_weights:
        ...
        for i in _range(0, len(a), BLOCK):
            ...
            if ntype.kind == 'c':
                n.real += np.bincount(
                    indices, weights=tmp_w.real,
                    minlength=n_equal_bins,
                )
                n.imag += np.bincount(
                    indices, weights=tmp_w.imag,
                    minlength=n_equal_bins,
                )
            else:
                n += np.bincount(
                    indices, weights=tmp_w,
                    minlength=n_equal_bins,
                ).astype(ntype)
    else:
...
def _histogramdd_dispatcher(
    sample, bins=None, range=None, density=None,
    weights=None,
):
