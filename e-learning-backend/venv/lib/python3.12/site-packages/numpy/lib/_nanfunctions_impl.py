...
def _remove_nan_1d(arr1d, second_arr1d=None, overwrite_input=False):
    ...
    if s.size == arr1d.size:
        warnings.warn(
            'All-NaN slice encountered', RuntimeWarning,
            stacklevel=6,
        )
...
def nanmin(
    ...
    if type(a) is np.ndarray and a.dtype != np.object_:
        ...
        if np.any(mask):
            ...
            warnings.warn(
                'All-NaN axis encountered', RuntimeWarning,
                stacklevel=2,
            )
    return res
...
def nanmedian(a, axis=None, out=None, overwrite_input=False, keepdims=np._NoValue):
    ...
    return fnb._ureduce(
        a, func=_nanmedian, keepdims=keepdims,
        axis=axis, out=out,
...
def nanvar(
    ...
    *, where=np._NoValue, mean=np._NoValue, correction=np._NoValue,
    ...
    if np.any(isbad):
        warnings.warn(
            'Degrees of freedom <= 0 for slice.', RuntimeWarning,
            stacklevel=2,
        )
