...
from ._iotools import _decode_line
from ._iotools import _is_string_like
from ._iotools import ConversionWarning
from ._iotools import ConverterError
from ._iotools import ConverterLockError
from ._iotools import easy_dtype
from ._iotools import flatten_dtype
from ._iotools import has_nested_fields
from ._iotools import LineSplitter
from ._iotools import NameValidator
from ._iotools import StringConverter
...
def load(
    ...
    with contextlib.ExitStack() as stack:
        ...
        if magic.startswith((_ZIP_PREFIX, _ZIP_SUFFIX)):
            ...
            stack.pop_all()
            ret = NpzFile(
                fid, own_fid=own_fid, allow_pickle=allow_pickle,
...
def save(file, arr, allow_pickle=True, fix_imports=np._NoValue):
    ...
    with file_ctx as fid:
        ...
        format.write_array(
            fid, arr, allow_pickle=allow_pickle,
...
def loadtxt(
    ...
    arr = _read(
        fname, dtype=dtype, comment=comment, delimiter=delimiter,
...
def _savetxt_dispatcher(
    fname, X, fmt=None, delimiter=None, newline=None,
    ...
    * Custom converters may receive unexpected values due to dtype
    ...
    with fid_ctx:
        ...
        if dtype is not None:
            dtype = easy_dtype(
                dtype, defaultfmt=defaultfmt, names=names,
                excludelist=excludelist,
                deletechars=deletechars,
                case_sensitive=case_sensitive,
                replace_space=replace_space,
...
#        else:
    ...
    if usemask and names:
        for (name, conv) in zip(names, converters):
            missing_values = [
                conv(_) for _ in conv.missing_values
                if _ != ''
            ]
