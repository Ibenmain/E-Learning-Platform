...
from numpy.lib._nanfunctions_impl import _nan_mask
from numpy.lib._nanfunctions_impl import _replace_nan
    ...
    def test_subclass(self):
        ...
        for f in self.nanfuncs:
            ...
            with warnings.catch_warnings(record=True) as w:
                ...
                assert_(isinstance(res, MyNDArray))
                assert_(
                    np.isnan(res[1]) and not np.isnan(res[0])
...
class TestNanFunctions_Percentile:
    ...
    def test_result_values(self, weighted, use_out):
        ...
        tgt = np.transpose([
            percentile(d, (28, 98), weights=gen_weights(d))
            for d in _rdat
        ])
        out = np.empty_like(tgt) if use_out else None
        res = nanpercentile(
            ...
            for keepdim in [False, True]:
                with suppress_warnings() as sup:
                    ...
                    nan_val = np.nanpercentile(
                        nan_mat, perc, axis=axis,
                        keepdims=keepdim,
                    )
                    assert_equal(nan_val.shape, val.shape)

                    val = np.percentile(
                        large_mat, perc, axis=axis,
                        keepdims=keepdim,
                    )
                    nan_val = np.nanpercentile(
                        large_mat, perc, axis=axis,
...
class TestNanFunctions_Quantile:
    ...
    def test_regression(self, weighted):
        ...
        assert_equal(
            np.nanquantile(ar, q=0.5, **w_args),
