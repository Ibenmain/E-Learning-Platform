...
def test_pickle_python2_python3():
    ...
    for fname in [
        'py2-np0-objarr.npy', 'py2-objarr.npy', 'py2-objarr.npz',
        ...
        if fname.startswith('py2'):
            if fname.endswith('.npz'):
                ...
                data.close()
                data = np.load(
                    path, allow_pickle=True, fix_imports=False,
                    encoding='latin1',
                )
                ...
                assert_raises(
                    UnicodeError, np.load, path,
                    allow_pickle=True,
                )
                assert_raises(
                    ImportError, np.load, path,
...
def test_pickle_disallow(tmpdir):
    ...
    assert_raises(
        ValueError, np.save, path, np.array([None], dtype=object),
        allow_pickle=False,
    )
...
def test_version_2_0_memmap(tmpdir):
    ...
    # 1.0 requested but data cannot be saved this way
    assert_raises(
        ValueError, format.open_memmap, tf1, mode='w+', dtype=d.dtype,
        shape=d.shape, version=(1, 0),
    )

    ma = format.open_memmap(
        tf1, mode='w+', dtype=d.dtype,
