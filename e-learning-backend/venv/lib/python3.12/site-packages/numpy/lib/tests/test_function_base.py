...
from numpy.testing import NOGIL_BUILD
...
class TestSelect:
    ...
    def test_return_dtype(self):
        assert_equal(
            select(self.conditions, self.choices, 1j).dtype,
            np.complex128,
        )
...
class TestInsert:
    ...
    def test_basic(self):
        ...
        assert_equal(
            insert(a, np.array([True, False, True, False]), 9),
            [9, 1, 2, 9, 3],
        )
    ...
    def test_multidim(self):
        ...
        assert_equal(
            insert(a, 1, a[:, :, 3], axis=-1),
            insert(a, 1, a[:, :, 3], axis=2),
        )
        assert_equal(
            insert(a, 1, a[:, 2, :], axis=-2),
...
class TestDelete:
    ...
    def _check_inverse_of_slicing(self, indices):
        ...
        assert_array_equal(
            setxor1d(a_del, self.a[indices,]), self.a,
            err_msg=msg,
        )
...
class TestAngle:
    ...
    def test_basic(self):
        x = [
            1 + 3j, np.sqrt(2) / 2.0 + 1j * np.sqrt(2) / 2,
            1, 1j, -1, -1j, 1 - 3j, -1 + 3j,
        ]
        y = angle(x)
        yo = [
...
class TestVectorize:
    ...
    def test_UnboundMethod_ticket_1156(self):
        ...
        assert_array_equal(
            vectorize(Foo().bar)(np.arange(9)),
            np.arange(9) ** 2,
        )
        assert_array_equal(
            vectorize(Foo.bar)(Foo(), np.arange(9)),
            np.arange(9) ** 2,
        )
...
class TestCov:
    ...
    def test_fweights(self):
        assert_allclose(
            cov(self.x2, fweights=self.frequencies),
            cov(self.x2_repeats),
        )
        assert_allclose(
            cov(self.x1, fweights=self.frequencies),
            self.res2,
        )
        assert_allclose(
    ...
    def test_unit_fweights_and_aweights(self):
        assert_allclose(
            cov(
                self.x2, fweights=self.frequencies,
        ...
        assert_allclose(
            cov(
                self.x1, fweights=self.frequencies,
                aweights=self.unit_weights,
            ),
            self.res2,
        )
        assert_allclose(
            cov(
                self.x1, fweights=self.unit_frequencies,
                aweights=self.unit_weights,
            ),
            self.res1,
        )
...
class TestPiecewise:
    ...
    def test_simple(self):
        ...
        assert_raises_regex(
            ValueError, '1 or 2 functions are expected',
    ...
    def test_0d_comparison(self):
        ...
        assert_raises_regex(
            ValueError, '2 or 3 functions are expected',
...
class TestBincount:
    ...
    def test_empty_list(self, minlength):
        assert_array_equal(
            np.bincount([], minlength=minlength),
...
class TestInterp:
    ...
    def test_non_finite_any_nan(self, sc):
        ...
        assert_equal(
            np.interp(0.5, [np.nan,      1], sc([0,     10])), sc(np.nan))
        assert_equal(np.interp(0.5, [0, np.nan], sc([0,     10])), sc(np.nan))
        assert_equal(np.interp(0.5, [0,      1],
                     sc([np.nan,     10])), sc(np.nan))
    ...
    def test_non_finite_inf(self, sc):
        ...
        assert_equal(np.interp(0.5, [0,       1],
                     sc([-np.inf, +np.inf])), sc(np.nan))
        assert_equal(np.interp(0.5, [0,       1],
                     sc([+np.inf, -np.inf])), sc(np.nan))

        # unless the y values are equal
        assert_equal(
            np.interp(0.5, [-np.inf, +np.inf], sc([10,      10])), sc(10))
    ...
    def test_complex_interp(self):
        ...
        y = [
            7.5+1.5j, 5.+1.0j, 8.75+1.75j, 6.25+1.25j, 3.+3j, 3.25+3.25j,
            3.5+3.5j, 3.75+3.75j,
        ]
...
class TestPercentile:
    ...
    def test_midpoint(self, dtype):
        assert_equal(
            np.percentile(
                np.arange(10, dtype=dtype), 51,
                method='midpoint',
            ), 4.5,
        )
        assert_equal(
            np.percentile(
                np.arange(9, dtype=dtype) + 1, 50,
                method='midpoint',
            ), 5,
        )
        assert_equal(
            np.percentile(
                np.arange(11, dtype=dtype), 51,
                method='midpoint',
            ), 5.5,
        )
        assert_equal(
            np.percentile(
                np.arange(11, dtype=dtype), 50,
                method='midpoint',
            ), 5,
        )
    ...
    def test_exception(self):
        assert_raises(
            ValueError, np.percentile, [1, 2], 56,
            method='foobar',
        )
    ...
    def test_percentile_empty_dim(self):
        ...
        assert_array_equal(
            np.array(np.percentile(d, [10, 50], axis=0)).shape,
            (2, 1, 2, 1),
        )
        assert_array_equal(
            np.array(np.percentile(d, [10, 50], axis=1)).shape,
            (2, 11, 2, 1),
        )
        assert_array_equal(
            np.array(np.percentile(d, [10, 50], axis=2)).shape,
            (2, 11, 1, 1),
        )
        assert_array_equal(
            np.array(np.percentile(d, [10, 50], axis=3)).shape,
            (2, 11, 1, 2),
        )
    ...
    def test_keepdims(self):
        d = np.ones((3, 5, 7, 11))
        assert_equal(
            np.percentile(d, 7, axis=None, keepdims=True).shape,
            (1, 1, 1, 1),
        )
        assert_equal(
        ...
        assert_equal(
            np.percentile(d, 7, axis=(1,), keepdims=True).shape,
            (3, 1, 7, 11),
        )
        assert_equal(
    ...
    def test_nan_behavior(self):
        ...
        assert_equal(
            np.percentile(a, [0.3, 0.6], axis=0),
            np.array([np.nan] * 2),
        )
...
class TestQuantile:
    ...
    def test_quantile_monotonic(self, method):
        ...
        quantile = np.quantile(
            np.array([
                0, 1, 1, 2, 2, 3, 3, 4, 5, 5, 1, 1, 9, 9, 9,
                8, 8, 7,
            ]) * 0.1, p0, method=method,
        )
    ...
    def test_quantile_identification_equation(self, weights, method, alpha):
        ...
        if method in ('higher',):
            ...
            # "sample precision".
            assert_allclose(
                np.average(self.V(x, y, alpha), weights=w), 0,
    ...
    def test_quantile_add_and_multiply_constant(self, weights, method, alpha):
        ...
        # 1
        assert_allclose(
            np.quantile(c + y, alpha, method=method, weights=w),
            c + q,
        )
        # 2
        assert_allclose(
...
class TestMedian:
    ...
    def test_overwrite_keyword(self):
        ...
        assert_allclose(
            np.median(a2.copy(), overwrite_input=True, axis=0),
            [1.5,  2.5,  3.5],
        )
        assert_allclose(
        ...
        assert_allclose(
            np.median(a3.T.copy(), overwrite_input=True, axis=1),
            [3,  4],
        )
    ...
    def test_extended_axis(self):
        ...
        np.random.shuffle(d.ravel())
        assert_equal(
            np.median(d, axis=(0, 1, 2))[0],
            np.median(d[:, :, :, 0].flatten()),
        )
        assert_equal(
            np.median(d, axis=(0, 1, 3))[1],
        ...
        assert_equal(
            np.median(d, axis=(3, 1, 2))[2],
            np.median(d[2, :, :, :].flatten()),
        )
        assert_equal(
            np.median(d, axis=(3, 2))[2, 1],
    ...
    def test_keepdims(self):
        d = np.ones((3, 5, 7, 11))
        assert_equal(
            np.median(d, axis=None, keepdims=True).shape,
            (1, 1, 1, 1),
        )
        assert_equal(
            np.median(d, axis=(0, 1), keepdims=True).shape,
            (1, 1, 7, 11),
        )
        assert_equal(
            np.median(d, axis=(0, 3), keepdims=True).shape,
            (1, 5, 7, 1),
        )
        assert_equal(
            np.median(d, axis=(1,), keepdims=True).shape,
            (3, 1, 7, 11),
        )
        assert_equal(
            np.median(d, axis=(0, 1, 2, 3), keepdims=True).shape,
            (1, 1, 1, 1),
        )
        assert_equal(
            np.median(d, axis=(0, 1, 3), keepdims=True).shape,
            (1, 1, 7, 1),
        )
