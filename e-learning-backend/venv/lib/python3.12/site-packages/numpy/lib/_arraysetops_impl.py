...
def _unique_dispatcher(
    ar, return_index=None, return_inverse=None,
...
def _unique1d(
    ...
    mask[:1] = True
    if (
        equal_nan and aux.shape[0] > 0 and aux.dtype.kind in 'cfmM' and
        np.isnan(aux[-1])
    ):
...
def _in1d_dispatcher(
    ar1, ar2, assume_unique=None, invert=None, *,
    kind=None,
):
    return (ar1, ar2)
    ...
    if use_table_method:
        ...
        if (
            ...
            outgoing_array[basic_mask] = isin_helper_ar[
                np.subtract(
                    in_range_ar1, ar2_min, dtype=dtype,
...
def _isin_dispatcher(
    element, test_elements, assume_unique=None, invert=None,
    *, kind=None,
):
...
def isin(
    ...
    return _in1d(
        element, test_elements, assume_unique=assume_unique,
