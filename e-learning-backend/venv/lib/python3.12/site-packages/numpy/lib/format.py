...
def descr_to_dtype(descr):
    ...
    return numpy.dtype({
        'names': names, 'formats': formats, 'titles': titles,
...
def _write_array_header(fp, d, version=None):
    ...
    for key, value in sorted(d.items()):
        ...
        header.append("'{}': {}, ".format(key, repr(value)))
    header.append('}')
    header = ''.join(header)
...
def _read_array_header(fp, version, max_header_size=_MAX_HEADER_SIZE):
    ...
    if (
        not isinstance(d['shape'], tuple) or
    ...
    if array.dtype.hasobject or not dtype_class._legacy:
        ...
        if not allow_pickle:
            if array.dtype.hasobject:
                raise ValueError(
                    'Object arrays cannot be saved when '
                    'allow_pickle=False',
                )
            if not dtype_class._legacy:
                raise ValueError(
                    'User-defined dtypes cannot be saved '
...
def open_memmap(
    ...
    marray = numpy.memmap(
        filename, dtype=dtype, shape=shape, order=order,
