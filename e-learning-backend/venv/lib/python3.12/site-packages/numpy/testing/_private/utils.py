...
from numpy._core import empty
from numpy._core import float32
from numpy._core import intp
from numpy._core import isnat
...
def assert_equal(actual, desired, err_msg='', verbose=True, *, strict=False):
    ...
    try:
        ...
        if (
            array_actual.dtype.char in 'Mm' or
...
def assert_almost_equal(actual, desired, decimal=7, err_msg='', verbose=True):
    ...
    def _build_err_msg():
        ...
        return build_err_msg(
            [actual, desired], err_msg, verbose=verbose,
            header=header,
        )
...
def assert_array_compare(
    ...
    *, strict=False, names=('ACTUAL', 'DESIRED'),
    ...
    from numpy._core import (
        array2string, isnan, inf, errstate,
        all, max, object_,
    )
    ...
    try:
        ...
        if isnumber(x) and isnumber(y):
            ...
            if equal_nan and dt == y.dtype and hasattr(dt, 'na_object'):
                is_nan = (
                    isinstance(dt.na_object, float) and
        ...
        msg = build_err_msg(
            [x, y], err_msg, verbose=verbose, header=header,
...
def assert_array_equal(
    ...
    assert_array_compare(
        operator.__eq__, actual, desired, err_msg=err_msg,
...
def assert_array_almost_equal(
    ...
    assert_array_compare(
        compare, actual, desired, err_msg=err_msg,
        verbose=verbose,
        header=('Arrays are not almost equal to %d decimals' % decimal),
        precision=decimal,
    )
...
def assert_array_less(x, y, err_msg='', verbose=True, *, strict=False):
    ...
    __tracebackhide__ = True  # Hide traceback for py.test
    assert_array_compare(
        operator.__lt__, x, y, err_msg=err_msg,
        verbose=verbose,
...
def assert_allclose(
    ...
    assert_array_compare(
        compare, actual, desired, err_msg=str(err_msg),
...
def nulp_diff(x, y, dtype=None):
    ...
    if not x.shape == y.shape:
        raise ValueError(
            'Arrays do not have the same shape: %s - %s' %
            (x.shape, y.shape),
        )
...
class suppress_warnings:
    ...
    def filter(self, category=Warning, message='', module=None):
        ...
        """
        self._filter(
            category=category, message=message, module=module,
            record=False,
        )
    ...
    def _showwarning(
        ...
        *args, use_warnmsg=None, **kwargs,
        ...
        if use_warnmsg is None:
            self._orig_show(
                message, category, filename, lineno, *args,
                **kwargs,
            )
        else:
            self._orig_showmsg(use_warnmsg)
