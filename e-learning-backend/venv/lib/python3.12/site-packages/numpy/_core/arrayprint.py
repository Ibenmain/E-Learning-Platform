...
from .multiarray import (
    array, dragon4_positional, dragon4_scientific,
    datetime_as_string, datetime_data, ndarray,
)
from .fromnumeric import any
from .numeric import concatenate, asarray, errstate
from .numerictypes import (
    longlong, intc, int_, float64, complex128,
    flexible,
)
...
def _leading_trailing(a, edgeitems, index=()):
    ...
    if a.shape[axis] > 2*edgeitems:
        return concatenate(
            (
                _leading_trailing(a, edgeitems, index +
                                  np.index_exp[:edgeitems]),
                _leading_trailing(a, edgeitems, index +
                                  np.index_exp[-edgeitems:]),
            ), axis=axis,
        )
...
def _array2string(a, options, separator=' ', prefix=''):
    ...
    lst = _formatArray(
        a, format_function, options['linewidth'],
...
def _extendLine_pretty(s, line, word, line_width, next_line_prefix, legacy):
    ...
    if (
        len(line) + max_word_length > line_width and
...
def format_float_scientific(
    ...
    return dragon4_scientific(
        x, precision=precision, unique=unique,
...
def format_float_positional(
    ...
    if not fractional and precision == 0:
        raise ValueError(
            'precision must be greater than 0 if '
            'fractional=False',
        )
    if min_digits > 0 and precision > 0 and min_digits > precision:
        raise ValueError('min_digits must be less than or equal to precision')
...
def _array_repr_implementation(
    ...
    prefix = class_name + '('
    if (
        current_options['legacy'] <= 113 and
...
def _array_str_implementation(
    ...
    if (
        format_options.get()['legacy'] <= 113 and
