...
class TestFloatExceptions:
    def assert_raises_fpe(self, fpeerr, flop, x, y):
        ...
        try:
            ...
            assert_(
                False,
                "Type {} did not raise fpe error '{}'.".format(ftype, fpeerr),
            )
        except FloatingPointError as exc:
            assert_(
                str(exc).find(fpeerr) >= 0,
                "Type {} raised wrong fpe error '{}'.".format(ftype, exc),
            )
...
class TestTypes:
    def check_promotion_cases(self, promote_func):
        ...
        assert_equal(
            promote_func(np.int32(-1), np.array([u64])),
            np.dtype(np.float64),
        )
        assert_equal(promote_func(f64, np.array([f32])), np.dtype(np.float64))
        assert_equal(
            promote_func(fld, np.array([f32])),
            np.dtype(np.longdouble),
        )
        assert_equal(
            promote_func(np.array([f64]), fld),
            np.dtype(np.longdouble),
        )
        assert_equal(
            promote_func(fld, np.array([c64])),
        ...
        assert_equal(
            promote_func(np.array([f32]), c128),
            np.dtype(np.complex128),
        )
    ...
    def test_coercion(self):
        ...
        for a in [np.array([True, False]), np.array([-3, 12], dtype=np.int8)]:
            ...
            assert_equal(
                b.dtype, np.dtype(np.longdouble),
            ...
            assert_equal(
                b.dtype, np.dtype(np.clongdouble),
                'array type %s' % a.dtype,
            )
...
class TestNonzero:
    ...
    def test_count_nonzero_axis_all_dtypes(self):
        ...
        for dt in np.typecodes['All']:
            ...
            if dt != 'V':
                ...
                expected = np.array(2)
                assert_equal(
                    np.count_nonzero(m, axis=(0, 1)),
                    expected, err_msg=err_msg,
                )
                assert_equal(
                    np.count_nonzero(m, axis=None),
                    expected, err_msg=err_msg,
                )
                assert_equal(
                    np.count_nonzero(m),
    ...
    def test_countnonzero_keepdims(self):
        ...
        assert_equal(
            np.count_nonzero(a, axis=0, keepdims=True),
            [[1, 2, 3, 0]],
        )
        assert_equal(
...
class TestBinaryRepr:
    ...
    def test_positive(self):
        ...
        assert_equal(
            np.binary_repr(12522),
            '11000011101010',
        )
        assert_equal(
            np.binary_repr(10736848),
    ...
    def test_large_neg_int64(self):
        # See gh-14289.
        assert_equal(
            np.binary_repr(np.int64(-2**62), width=64),
            '11' + '0'*62,
        )
...
def assert_array_strict_equal(x, y):
    ...
    if (
        (
            x.dtype.alignment <= 8 or
            np.intp().dtype.itemsize != 4
        ) and
        sys.platform != 'win32'
    ):
...
class TestAllclose:
    ...
    def tst_allclose(self, x, y):
        assert_(np.allclose(x, y), '{} and {} not close'.format(x, y))

    def tst_not_allclose(self, x, y):
        assert_(not np.allclose(x, y),
                "{} and {} shouldn't be close".format(x, y))
...
class TestIsclose:
    ...
    def tst_all_isclose(self, x, y):
        assert_(np.all(np.isclose(x, y)), '{} and {} not close'.format(x, y))
...
class TestMoveaxis:
    ...
    def test_errors(self):
        ...
        assert_raises_regex(
            AxisError, 'source.*out of bounds',
            np.moveaxis, x, 3, 0,
        )
        assert_raises_regex(
            AxisError, 'source.*out of bounds',
        ...
        assert_raises_regex(
            AxisError, 'destination.*out of bounds',
            np.moveaxis, x, 0, 5,
        )
        assert_raises_regex(
            ValueError, 'repeated axis in `source`',
        ...
        assert_raises_regex(
            ValueError, 'must have the same number',
            np.moveaxis, x, 0, [0, 1],
        )
        assert_raises_regex(
            ValueError, 'must have the same number',
...
class TestBroadcast:
    def test_broadcast_in_args(self):
        # gh-5881
        arrs = [
            np.empty((6, 7)), np.empty((5, 6, 1)), np.empty((7,)),
            np.empty((5, 1, 7)),
        ]
        mits = [
            np.broadcast(*arrs),
    ...
    def test_shape_mismatch_error_message(self):
        with pytest.raises(
            ValueError, match=r'arg 0 with shape \(1, 3\) and '
