...
class TestRecord:
    ...
    def test_union_struct(self):
        ...
        dt = np.dtype(
            {
                'names': ['f0', 'f1', 'f2'], 'formats': ['<u4', '<u2', '<u2'],
                'offsets': [0, 0, 2],
            }, align=True,
        )
...
class TestSubarray:
    ...
    def test_shape_equal(self):
        ...
        assert_dtype_equal(np.dtype(('<f4', (3, 2))),
                           np.dtype(('<f4', (3, 2))))
    ...
    def test_shape_simple(self):
        ...
        assert_dtype_not_equal(np.dtype(('f8', (1,))),
                               np.dtype(('f8', (1, 1))))
        assert_dtype_not_equal(np.dtype(('f4', (3, 2))),
                               np.dtype(('f4', (2, 3))))
...
class TestStructuredObjectRefcounting:
    ...
    def test_structured_object_indexing(
        self, shape, index, items_changed,
        dt, pat, count, singleton,
    ):
...
class TestDtypeAttributeDeletion:
    ...
    def test_dtype_non_writable_attributes_deletion(self):
        ...
        attr = [
            'subdtype', 'descr', 'str', 'name', 'base', 'shape',
...
class TestPickling:
    ...
    @pytest.mark.parametrize(
        't', [
            int, float, complex, np.int32, str, object,
            bool,
        ],
    )
    ...
    def test_datetime(self, base, unit):
        dt = np.dtype('{}[{}]'.format(base, unit) if unit else base)
        self.check_pickling(dt)
        if unit:
            dt = np.dtype('{}[7{}]'.format(base, unit))
            self.check_pickling(dt)
...
class TestDTypeClasses:
    ...
    def test_basic_dtypes_subclass_properties(self, dtype):
        ...
        parametric = (np.void, np.str_, np.bytes_,
...
class TestFromCTypes:
    ...
    def test_union_with_struct_packed(self):
        ...
        expected = np.dtype(
            dict(
                names=['a', 'b', 'c', 'd'],
                formats=['u1', np.uint16, np.uint32, [
        ...
        expected = np.dtype(
            dict(
                formats=[np.uint8, np.uint16, np.uint8,
                         np.uint16, np.uint32, np.uint32, np.uint8],
