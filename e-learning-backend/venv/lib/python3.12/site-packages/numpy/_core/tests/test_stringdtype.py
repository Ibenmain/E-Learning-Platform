    ...
    for dtypes in [('T', 'U'), ('U', 'T')]:
        for function, answer in [
            ...
            assert answer == function(
                np.array(l, dtype=dtypes[0]), np.array(
...
def test_center_promoter():
    ...
    for dtypes in [('T', 'U'), ('U', 'T')]:
        answer = np.strings.center(
            np.array(arg, dtype=dtypes[0]), 9, np.array(
...
def test_unset_na_coercion():
    ...
    for op_dtype in [
        None, StringDType(), StringDType(coerce=False),
    ...
    for op_dtype in [
        None, StringDType(), StringDType(coerce=True),
        StringDType(na_object=None),
    ]:
        if op_dtype is None:
            op = inp
...
class TestImplementation:
    ...
    def test_setup(self):
        ...
        sizes = np.where(
            is_short, view['size_and_flags'] & 0xf,
            view['size'],
        )
