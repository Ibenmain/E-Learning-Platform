...
class TestDateTime:
    ...
    def test_datetime_scalar_construction(self):
        ...
        assert_equal(
            repr(np.datetime64('NaT')),
            "np.datetime64('NaT')",
        )
        assert_equal(str(np.datetime64('2011-02')), '2011-02')
        assert_equal(
            repr(np.datetime64('2011-02')),
        ...
        assert_equal(
            np.datetime64('1920-03-13', 'h'),
            np.datetime64('1920-03-13T00'),
        )
        assert_equal(
            np.datetime64('1920-03', 'm'),
            np.datetime64('1920-03-01T00:00'),
        )
        assert_equal(
            np.datetime64('1920', 's'),
        ...
        assert_equal(
            np.datetime64('1920-03-13T18', 'D'),
            np.datetime64('1920-03-13'),
        )
        assert_equal(
            np.datetime64('1920-03-13T18:33:12', 'M'),
            np.datetime64('1920-03'),
        )
        assert_equal(
            np.datetime64('1920-03-13T18:33:12.5', 'Y'),
            np.datetime64('1920'),
        )
    ...
    def test_timedelta_np_int_construction(self, unit):
        ...
        if unit != 'generic':
            assert_equal(
                np.timedelta64(np.int64(123), unit),
                np.timedelta64(123, unit),
            )
        else:
            assert_equal(
                np.timedelta64(np.int64(123)),
                np.timedelta64(123),
            )
    ...
    def test_timedelta_scalar_construction(self):
        ...
        assert_equal(
            repr(np.timedelta64('NaT')),
            "np.timedelta64('NaT')",
        )
        assert_equal(str(np.timedelta64(3, 's')), '3 seconds')
        assert_equal(
            repr(np.timedelta64(-3, 's')),
            "np.timedelta64(-3,'s')",
        )
        assert_equal(
            repr(np.timedelta64(12)),
            'np.timedelta64(12)',
        )
    ...
    def test_timedelta_nat_format(self):
        # gh-17552
        assert_equal('NaT', '{}'.format(np.timedelta64('nat')))
    ...
    def test_timedelta_scalar_construction_units(self):
        ...
        assert_equal(
            np.datetime64('2010-03').dtype,
            np.dtype('M8[M]'),
        )
        assert_equal(
            np.datetime64('2010-03-12').dtype,
            np.dtype('M8[D]'),
        )
        assert_equal(
            np.datetime64('2010-03-12T17').dtype,
            np.dtype('M8[h]'),
        )
        assert_equal(
            np.datetime64('2010-03-12T17:15').dtype,
            np.dtype('M8[m]'),
        )
        assert_equal(
            np.datetime64('2010-03-12T17:15:08').dtype,
            np.dtype('M8[s]'),
        )

        assert_equal(
            np.datetime64('2010-03-12T17:15:08.1').dtype,
            np.dtype('M8[ms]'),
        )
        assert_equal(
            np.datetime64('2010-03-12T17:15:08.12').dtype,
            np.dtype('M8[ms]'),
        )
        assert_equal(
            np.datetime64('2010-03-12T17:15:08.123').dtype,
            np.dtype('M8[ms]'),
        )

        assert_equal(
            np.datetime64('2010-03-12T17:15:08.1234').dtype,
            np.dtype('M8[us]'),
        )
        assert_equal(
        ...
        assert_equal(
            np.datetime64(
                '1970-01-01T00:00:02.1234567890123',
            ).dtype,
            np.dtype('M8[fs]'),
        )
        assert_equal(
            np.datetime64(
                '1970-01-01T00:00:02.12345678901234',
            ).dtype,
            np.dtype('M8[fs]'),
        )
        assert_equal(
            np.datetime64(
                '1970-01-01T00:00:02.123456789012345',
            ).dtype,
            np.dtype('M8[fs]'),
        )
        ...
        assert_equal(
            np.datetime64(
                '1970-01-01T00:00:02.1234567890123456',
            ).dtype,
            np.dtype('M8[as]'),
        )
        assert_equal(
        ...
        assert_equal(
            np.datetime64(
                '1970-01-01T00:00:02.123456789012345678',
            ).dtype,
            np.dtype('M8[as]'),
        )
        ...
        # Python date object
        assert_equal(
            np.datetime64(datetime.date(2010, 4, 16)).dtype,
            np.dtype('M8[D]'),
        )
        ...
        assert_equal(
            np.datetime64(
                datetime.datetime(2010, 4, 16, 13, 45, 18),
            ).dtype,
            np.dtype('M8[us]'),
        )
        ...
        # 'today' special value
        assert_equal(
            np.datetime64('today').dtype,
            np.dtype('M8[D]'),
        )

        # 'now' special value
        assert_equal(
            np.datetime64('now').dtype,
            np.dtype('M8[s]'),
        )
    ...
    def test_pickle(self):
        ...
        for proto in range(2, pickle.HIGHEST_PROTOCOL + 1):
            ...
            assert_equal(
                pickle.loads(pickle.dumps(scalar, protocol=proto)),
                scalar,
            )
            delta = scalar - np.datetime64('2015-01-01T00:00:00.000000000')
            assert_equal(
                pickle.loads(pickle.dumps(delta, protocol=proto)),
                delta,
            )
    ...
    def test_dtype_promotion(self):
        ...
        assert_raises(
            OverflowError, np.promote_types,
    ...
    def test_pyobject_roundtrip(self):
        ...
        for unit in ['M8[D]', 'M8[W]', 'M8[M]', 'M8[Y]']:
            ...
            assert_equal(
                b.astype(object).astype(unit), b,
                'Error roundtripping unit %s' % unit,
            )
        # With time units
        for unit in [
            'M8[as]', 'M8[16fs]', 'M8[ps]', 'M8[us]',
            'M8[300as]', 'M8[20us]',
        ]:
            b = a.copy().view(dtype=unit)
            b[0] = '-0001-01-01T00'
    ...
    def test_different_unit_comparison(self):
        ...
        for unit1 in ['Y', 'M', 'D']:
            ...
            for unit2 in ['Y', 'M', 'D']:
                ...
                assert_equal(
                    np.array('1945', dtype=dt1),
                    np.array('1945', dtype=dt2),
                )
                assert_equal(
                    np.array('1970', dtype=dt1),
                    np.array('1970', dtype=dt2),
                )
                assert_equal(
                    np.array('9999', dtype=dt1),
                ...
                assert_equal(
                    np.datetime64('1945', unit1),
                    np.datetime64('1945', unit2),
                )
                assert_equal(
                    np.datetime64('1970', unit1),
                    np.datetime64('1970', unit2),
                )
                assert_equal(
                    np.datetime64('9999', unit1),
    ...
    def test_datetime_add(self):
        for dta, dtb, dtc, dtnat, tda, tdb, tdc in \
            ...
            assert_equal(
                np.add(dta, tdb, casting='unsafe').dtype,
                np.dtype('M8[h]'),
            )
            assert_equal(np.add(tdb, dta, casting='unsafe'), dtc)
            assert_equal(
                np.add(tdb, dta, casting='unsafe').dtype,
                np.dtype('M8[h]'),
            )
    ...
    def test_datetime_subtract(self):
        for dta, dtb, dtc, dtd, dte, dtnat, tda, tdb, tdc in \
            ...
            assert_equal(
                np.subtract(dtc, dtd, casting='unsafe'),
                np.timedelta64(0, 'h'),
            )
            assert_equal(
            ...
            assert_equal(
                np.subtract(dtd, dtc, casting='unsafe'),
                np.timedelta64(0, 'h'),
            )
            assert_equal(
    ...
    def test_string_parser_error_check(self):
        ...
        assert_raises(
            ValueError, np.array, ['1980-02-03%'],
            np.dtype('M8[us]'),
        )
        assert_raises(
            ValueError, np.array, ['1980-02-03 q'],
            np.dtype('M8[us]'),
        )

        # Hours must be in range [0, 23]
        assert_raises(
            ValueError, np.array, ['1980-02-03 25'],
            np.dtype('M8[us]'),
        )
        assert_raises(
            ValueError, np.array, ['1980-02-03T25'],
            np.dtype('M8[us]'),
        )
        assert_raises(
            ValueError, np.array, ['1980-02-03 24:01'],
            np.dtype('M8[us]'),
        )
        assert_raises(
            ValueError, np.array, ['1980-02-03T24:01'],
            np.dtype('M8[us]'),
        )
        assert_raises(
            ValueError, np.array, ['1980-02-03 -1'],
            np.dtype('M8[us]'),
        )
        # No trailing ':'
        assert_raises(
            ValueError, np.array, ['1980-02-03 01:'],
            np.dtype('M8[us]'),
        )
        # Minutes must be in range [0, 59]
        assert_raises(
            ValueError, np.array, ['1980-02-03 01:-1'],
            np.dtype('M8[us]'),
        )
        assert_raises(
            ValueError, np.array, ['1980-02-03 01:60'],
            np.dtype('M8[us]'),
        )
        # No trailing ':'
        assert_raises(
        ...
        assert_raises(
            ValueError, np.array, ['1980-02-03 01:10:-1'],
            np.dtype('M8[us]'),
        )
        assert_raises(
            ValueError, np.array, ['1980-02-03 01:01:60'],
            np.dtype('M8[us]'),
        )
        # Timezone offset must within a reasonable range
        with pytest.warns(UserWarning, match=msg):
            assert_raises(
                ValueError, np.array, ['1980-02-03 01:01:00+0661'],
                np.dtype('M8[us]'),
            )
        with pytest.warns(UserWarning, match=msg):
            assert_raises(
                ValueError, np.array, ['1980-02-03 01:01:00+2500'],
                np.dtype('M8[us]'),
            )
        with pytest.warns(UserWarning, match=msg):
            assert_raises(
                ValueError, np.array, ['1980-02-03 01:01:00-0070'],
                np.dtype('M8[us]'),
            )
        with pytest.warns(UserWarning, match=msg):
            assert_raises(
                ValueError, np.array, ['1980-02-03 01:01:00-3000'],
                np.dtype('M8[us]'),
            )
        with pytest.warns(UserWarning, match=msg):
            assert_raises(
                ValueError, np.array, ['1980-02-03 01:01:00-25:00'],
                np.dtype('M8[us]'),
            )
    ...
    def test_datetime_as_string(self):
        ...
        assert_equal(
            np.datetime_as_string(np.datetime64(date, 'Y')),
            '1959',
        )
        assert_equal(
            np.datetime_as_string(np.datetime64(date, 'M')),
            '1959-10',
        )
        assert_equal(
            np.datetime_as_string(np.datetime64(date, 'D')),
            '1959-10-13',
        )
        assert_equal(
        ...
        assert_equal(
            np.datetime_as_string(np.datetime64(datetime, 'as')),
            datetime,
        )
        ...
        assert_equal(
            np.datetime_as_string(a, unit='Y', casting='unsafe'),
            '2032',
        )
        assert_equal(
            np.datetime_as_string(a, unit='M', casting='unsafe'),
            '2032-07',
        )
        assert_equal(
            np.datetime_as_string(a, unit='W', casting='unsafe'),
            '2032-07-18',
        )
        assert_equal(
            np.datetime_as_string(a, unit='D', casting='unsafe'),
            '2032-07-18',
        )
        assert_equal(np.datetime_as_string(a, unit='h'), '2032-07-18T12')
        assert_equal(
            np.datetime_as_string(a, unit='m'),
            '2032-07-18T12:23',
        )
        assert_equal(
            np.datetime_as_string(a, unit='s'),
            '2032-07-18T12:23:34',
        )
        assert_equal(
            np.datetime_as_string(a, unit='ms'),
            '2032-07-18T12:23:34.123',
        )
        assert_equal(
            np.datetime_as_string(a, unit='us'),
    ...
    def test_datetime_as_string_timezone(self):
        ...
        assert_equal(
            np.datetime_as_string(a, timezone='naive'),
            '2010-03-15T06:30',
        )
        assert_equal(
            np.datetime_as_string(a, timezone='UTC'),
            '2010-03-15T06:30Z',
        )
        assert_(
        ...
        assert_raises(
            TypeError, np.datetime_as_string, a, unit='D',
    ...
    def test_datetime_arange(self):
        ...
        assert_equal(
            ...
            np.array(
                [
                    '1950-02-10', '1950-02-09', '1950-02-08',
                    '1950-02-07',
                ], dtype='M8[D]',
            ),
        )
        ...
        assert_raises(
            ValueError, np.arange, np.datetime64('today'),
            np.datetime64('today') + 3, 0,
        )
        # Promotion across nonlinear unit boundaries is disallowed
        assert_raises(
            TypeError, np.arange, np.datetime64('2011-03-01', 'D'),
            np.timedelta64(5, 'M'),
        )
        assert_raises(
            TypeError, np.arange,
    ...
    def test_timedelta_arange(self):
        ...
        # Step of 0 is disallowed
        assert_raises(
            ValueError, np.arange, np.timedelta64(0),
            np.timedelta64(5), 0,
        )
        # Promotion across nonlinear unit boundaries is disallowed
        assert_raises(
            TypeError, np.arange, np.timedelta64(0, 'D'),
            np.timedelta64(5, 'M'),
        )
        assert_raises(
    ...
    def test_datetime_busday_offset(self):
        ...
        assert_equal(
            np.busday_offset('2006-02-01', 25),
            np.datetime64('2006-03-08'),
        )
        assert_equal(
            np.busday_offset('2006-03-08', -25),
    ...
    def test_datetime_busday_holidays_offset(self):
        ...
        assert_raises(
            ValueError, np.busday_offset, '2012-01-03', -15,
    ...
    def test_datetime_busday_holidays_count(self):
        holidays = [
            '2011-01-01', '2011-10-10', '2011-11-11', '2011-11-24',
        ...
        dates = np.busday_offset(
            '2011-01-01', np.arange(366),
            roll='forward', busdaycal=bdd,
        )
        assert_equal(
            np.busday_count('2011-01-01', dates, busdaycal=bdd),
            np.arange(366),
        )
        ...
        expected[0] = -1
        assert_equal(
            np.busday_count(dates, '2011-12-31', busdaycal=bdd),
            expected,
        )
        # Returns negative value when reversed
        ...
        expected[0] = 0
        assert_equal(
            np.busday_count('2011-12-31', dates, busdaycal=bdd),
            expected,
        )

        # Can't supply both a weekmask/holidays and busdaycal
        assert_raises(
            ValueError, np.busday_offset, '2012-01-03', '2012-02-03',
            weekmask='1111100', busdaycal=bdd,
        )
        assert_raises(
            ValueError, np.busday_offset, '2012-01-03', '2012-02-03',
