...
def test_float_types(tp):
    ...
    for x in [0, 1, -1, 1e20]:
        assert_equal(
            str(tp(x)), str(float(x)),
            err_msg='Failed str formatting for type %s' % tp,
        )

    if tp(1e16).itemsize > 4:
        assert_equal(
            str(tp(1e16)), str(float('1e16')),
            err_msg='Failed str formatting for type %s' % tp,
        )
    else:
        ref = '1e+16'
...
def test_complex_types(tp):
    ...
    for x in [0, 1, -1, 1e20]:
        assert_equal(
            str(tp(x)), str(complex(x)),
            err_msg='Failed str formatting for type %s' % tp,
        )
        assert_equal(
            str(tp(x*1j)), str(complex(x*1j)),
            err_msg='Failed str formatting for type %s' % tp,
        )
        assert_equal(
            str(tp(x + x*1j)), str(complex(x + x*1j)),
            err_msg='Failed str formatting for type %s' % tp,
        )

    if tp(1e16).itemsize > 8:
        assert_equal(
            str(tp(1e16)), str(complex(1e16)),
            err_msg='Failed str formatting for type %s' % tp,
        )
    else:
        ref = '(1e+16+0j)'
...
def _test_redirected_print(x, tp, ref=None):
    ...
    assert_equal(
        file.getvalue(), file_tp.getvalue(),
    ...
    for (fmat, val, valtype) in tests:
        try:
            assert_equal(
                ...
                'failed with val {}, type {}'.format(val, valtype),
            )
