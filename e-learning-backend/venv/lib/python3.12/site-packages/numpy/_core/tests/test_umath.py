...
from numpy.testing import _gen_alignment_data
...
from numpy.testing import IS_MUSL
...
UFUNCS = [
    obj for obj in np._core.umath.__dict__.values()
...
class TestComparisons:
    ...
    @pytest.mark.parametrize(
        'dtype', sctypes['uint'] + sctypes['int'] +
...
class TestDivision:
    ...
    @pytest.mark.parametrize(
        'dtype,ex_val', itertools.product(
    ...
    @pytest.mark.skipif(
        hasattr(np.__config__, 'blas_ssl2_info'),
        reason='gh-22982',
    )
    @pytest.mark.skipif(IS_WASM, reason="fp errors don't work in wasm")
    ...
    def test_float_remainder_exact(self):
        ...
        for op in [floor_divide_and_remainder, np.divmod]:
            for dt in np.typecodes['Float']:
                msg = 'op: {}, dtype: {}'.format(op.__name__, dt)
                fa = a.astype(dt)
                fb = b.astype(dt)
                div, rem = op(fa, fb)
    ...
    def test_float_divmod_corner_cases(self):
        ...
        for dt in np.typecodes['Float']:
            ...
            with suppress_warnings() as sup:
                ...
                assert (np.isinf(div)), 'dt: {}, div: {}'.format(dt, rem)
                assert (np.isnan(rem)), 'dt: {}, rem: {}'.format(dt, rem)
                div, rem = np.divmod(fzer, fzer)
                assert (np.isnan(rem)), 'dt: {}, rem: {}'.format(dt, rem)
                assert_(np.isnan(div)), 'dt: {}, rem: {}'.format(dt, rem)
                div, rem = np.divmod(finf, finf)
                assert (np.isnan(div)), 'dt: {}, rem: {}'.format(dt, rem)
                assert (np.isnan(rem)), 'dt: {}, rem: {}'.format(dt, rem)
                div, rem = np.divmod(finf, fzer)
                assert (np.isinf(div)), 'dt: {}, rem: {}'.format(dt, rem)
                assert (np.isnan(rem)), 'dt: {}, rem: {}'.format(dt, rem)
    ...
    def test_float_remainder_corner_cases(self):
        ...
        with suppress_warnings() as sup:
            ...
            for dt in np.typecodes['Float']:
                ...
                assert_(np.isnan(rem), 'dt: {}, rem: {}'.format(dt, rem))
                # MSVC 2008 returns NaN here, so disable the check.
                # rem = np.remainder(fone, finf)
                # assert_(rem == fone, 'dt: %s, rem: %s' % (dt, rem))
                rem = np.remainder(finf, fone)
                fmod = np.fmod(finf, fone)
                assert_(np.isnan(fmod), 'dt: {}, fmod: {}'.format(dt, fmod))
                assert_(np.isnan(rem), 'dt: {}, rem: {}'.format(dt, rem))
                ...
                assert_(np.isnan(rem), 'dt: {}, rem: {}'.format(dt, rem))
                assert_(np.isnan(fmod), 'dt: {}, fmod: {}'.format(dt, rem))
...
class TestPower:
    ...
    def test_power_complex(self):
        ...
        assert_almost_equal(
            x**(-3), [
                (-11+2j)/125, (-46-9j)/2197,
                (-117-44j)/15625,
            ],
        )
        assert_almost_equal(
            x**(0.5), [
                ncu.sqrt(1+2j), ncu.sqrt(2+3j),
                ncu.sqrt(3+4j),
            ],
        )
        norm = 1./((x**14)[0])
        assert_almost_equal(
            ...
            [
                i * norm for i in [
                    -76443+16124j, 23161315+58317492j,
...
class TestFloat_power:
    def test_type_conversion(self):
        ...
        for dtin, dtout in zip(arg_type, res_type):
            msg = 'dtin: {}, dtout: {}'.format(dtin, dtout)
...
class TestLog:
    ...
    def test_log_strides(self):
        ...
        for ii in sizes:
            x_f64 = np.float64(np.random.uniform(
                low=0.01, high=100.0, size=ii))
...
class TestSpecialFloats:
    ...
    def test_log_values(self):
        ...
        with np.errstate(divide='raise'):
            for dt in ['e', 'f', 'd']:
                assert_raises(
                    FloatingPointError, np.log,
                    np.array(0.0, dtype=dt),
                )
                assert_raises(
                    FloatingPointError, np.log2,
                    np.array(0.0, dtype=dt),
                )
                assert_raises(
                    FloatingPointError, np.log10,
                    np.array(0.0, dtype=dt),
                )
                assert_raises(
                    FloatingPointError, np.log1p,
    ...
    def test_reciprocal_values(self):
        ...
        with np.errstate(divide='raise'):
            for dt in ['e', 'f', 'd', 'g']:
                assert_raises(
                    FloatingPointError, np.reciprocal,
                    np.array(-0.0, dtype=dt),
                )
...
class TestFPClass:
    ...
    def test_fpclass(self, stride):
        arr_f64 = np.array([np.nan, -np.nan, np.inf, -np.inf, -1.0,
                           1.0, -0.0, 0.0, 2.2251e-308, -2.2251e-308], dtype='d')
        arr_f32 = np.array([np.nan, -np.nan, np.inf, -np.inf, -1.0,
                           1.0, -0.0, 0.0, 1.4013e-045, -1.4013e-045], dtype='f')
    ...
    def test_fp_noncontiguous(self, dtype):
        data = np.array(
            [
                np.nan, -np.nan, np.inf, -np.inf, -1.0,
...
class TestAVXFloat32Transcendental:
    ...
    def test_sincos_float32(self):
        ...
        # test aliasing(issue #17761)
        tx_f32 = x_f32.copy()
        assert_array_max_ulp(np.sin(x_f32, out=x_f32),
                             np.float32(np.sin(x_f64)), maxulp=2)
        assert_array_max_ulp(np.cos(tx_f32, out=tx_f32),
...
def assert_hypot_isnan(x, y):
    with np.errstate(invalid='ignore'):
        assert_(
            ...
            'hypot({}, {}) is {}, not nan'.format(x, y, ncu.hypot(x, y)),
        )
...
def assert_hypot_isinf(x, y):
    with np.errstate(invalid='ignore'):
        assert_(
            ...
            'hypot({}, {}) is {}, not inf'.format(x, y, ncu.hypot(x, y)),
        )
...
def assert_arctan2_isnan(x, y):
    assert_(np.isnan(ncu.arctan2(x, y)),
            'arctan({}, {}) is {}, not nan'.format(x, y, ncu.arctan2(x, y)))


def assert_arctan2_ispinf(x, y):
    assert_((np.isinf(ncu.arctan2(x, y)) and ncu.arctan2(x, y) > 0),
            'arctan({}, {}) is {}, not +inf'.format(x, y, ncu.arctan2(x, y)))
...
def assert_arctan2_ispzero(x, y):
    assert_((ncu.arctan2(x, y) == 0 and not np.signbit(ncu.arctan2(x, y))),
            'arctan({}, {}) is {}, not +0'.format(x, y, ncu.arctan2(x, y)))
...
class TestMaximum(_FilterInvalids):
    ...
    def test_strided_array(self):
        ...
        arr2 = np.array(
            [-2.0, -1.0, np.nan, 1.0, 0.0,    np.nan, 1.0,    -3.0])
        maxtrue = np.array(
...
class TestAbsoluteNegative:
    ...
    def test_noncontiguous(self, dtype, big):
        data = np.array(
            [
                -1.0, 1.0, -0.0, 0.0, 2.2251e-308, -2.5, 2.5, -6,
                6, -2.2251e-308, -8, 10,
            ], dtype=dtype,
        )
        expect = np.array(
        ...
        # contig in, ncontig out
        assert_array_equal(
            np.negative(contig_in, out=ncontig_out),
            expect[1::2],
        )
        # ncontig in, contig out
        ...
        # ncontig in, ncontig out
        assert_array_equal(
            np.negative(ncontig_in, out=ncontig_out),
            expect[1::2],
        )
        # contig in, contig out, nd stride
        data_split = np.array(np.array_split(data, 2))
...
class TestSpecialMethods:
    ...
    def test_ufunc_override_methods(self):
        ...
        assert_equal(
            res[4], {
                'axis': 0, 'dtype': None, 'keepdims': False,
        ...
        assert_raises(
            ValueError, np.multiply.accumulate, a,
            out=('out0', 'out1'),
        )
        assert_raises(
            TypeError, np.multiply.accumulate, a,
            'axis0', axis='axis0',
        )
        ...
        assert_raises(
            ValueError, np.multiply.reduce, a, [4, 2],
            out=('out0', 'out1'),
        )
        assert_raises(
            TypeError, np.multiply.reduce, a, [4, 2],
            'axis0', axis='axis0',
        )
        ...
        assert_raises(TypeError, np.multiply.outer,
                      a, a, sig='a', signature='a')
...
class TestComplexFunctions:
    funcs = [
        np.arcsin,  np.arccos,  np.arctan, np.arcsinh, np.arccosh,
    ...
    def test_branch_cuts_complex64(self):
        ...
        _check_branch_cut(np.arccosh, [0-2j, 2j, 2],
                          [1,  1,  1j], 1, 1, False, np.complex64)
        _check_branch_cut(np.arctanh, [0-2j, 2j, 0],
                          [1,  1,  1j], 1, 1, False, np.complex64)
    ...
    def test_loss_of_precision(self, dtype):
        ...
        def check(x, rtol):
            ...
            assert_(
                np.all(d < rtol), (
                    np.argmax(d), x[np.argmax(d)], d.max(),
                    'arcsinh',
                ),
            )
        ...
        for func in (np.arcsinh, np.arcsinh, np.arcsin, np.arctanh, np.arctan):
            pts = [
                rp+1j*ip for rp in (-1e-3, 0, 1e-3) for ip in (-1e-3, 0, 1e-3)
                if rp != 0 or ip != 0
            ]
            check(func, pts, 1)
            check(func, pts, 1j)
...
def _check_branch_cut(
    ...
    if sig_zero_ok:
        ...
        if np.any(jr):
            ...
            ym = f(x)
            assert_(
                np.all(np.absolute(y0[jr].real - ym.real*re_sign) < atol), (y0[jr], ym))
            assert_(
                np.all(np.absolute(y0[jr].imag - ym.imag*im_sign) < atol), (y0[jr], ym))
        ...
        if np.any(ji):
            ...
            ym = f(x)
            assert_(
                np.all(np.absolute(y0[ji].real - ym.real*re_sign) < atol), (y0[ji], ym))
            assert_(
                np.all(np.absolute(y0[ji].imag - ym.imag*im_sign) < atol), (y0[ji], ym))
...
def test_nextafter_0():
    for t, direction in itertools.product(np._core.sctypes['float'], (1, -1)):
        ...
        assert_equal(np.nextafter(t(0), t(direction)) /
                     t(2.1), direction * 0.0)
    ...
    with np.errstate(invalid='ignore'):
        for x in nans + fins:
            ...
            for y in nans + fins:
                ...
                assert_equal(x < y, False, err_msg='{!r} < {!r}'.format(x, y))
                assert_equal(x > y, False, err_msg='{!r} > {!r}'.format(x, y))
                assert_equal(
                    x <= y, False, err_msg='{!r} <= {!r}'.format(x, y))
                assert_equal(
                    x >= y, False, err_msg='{!r} >= {!r}'.format(x, y))
                assert_equal(
                    x == y, False, err_msg='{!r} == {!r}'.format(x, y))
