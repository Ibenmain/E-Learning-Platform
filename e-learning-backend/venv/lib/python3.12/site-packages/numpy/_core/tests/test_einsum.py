...
class TestEinsum:
    ...
    def test_einsum_errors(self, do_opt, einsum_fn):
        ...
        assert_raises(
            ValueError, einsum_fn, 'ij->jij', [[0, 0], [0, 0]],
            optimize=do_opt,
        )

        # dimensions must match when being collapsed
        assert_raises(
            ValueError, einsum_fn, 'ii',
            np.arange(6).reshape(2, 3), optimize=do_opt,
        )
        assert_raises(
            ValueError, einsum_fn, 'ii->i',
            np.arange(6).reshape(2, 3), optimize=do_opt,
        )
    ...
    def test_einsum_specific_errors(self, do_opt):
        ...
        assert_raises(
            TypeError, np.einsum, '', 0, out='test',
            optimize=do_opt,
        )

        # order parameter must be a valid order
        assert_raises(
            ValueError, np.einsum, '', 0, order='W',
            optimize=do_opt,
        )

        # casting parameter must be a valid casting
        assert_raises(
            ValueError, np.einsum, '', 0, casting='blah',
            optimize=do_opt,
        )

        # dtype parameter must be a valid dtype
        assert_raises(
            TypeError, np.einsum, '', 0, dtype='bad_data_type',
            optimize=do_opt,
        )
        ...
        assert_raises(
            ValueError, np.einsum, 'i', np.arange(6).reshape(2, 3),
            optimize=do_opt,
        )
        assert_raises(
            ValueError, np.einsum, 'i->i', [[0, 1], [0, 1]],
            out=np.arange(4).reshape(2, 2), optimize=do_opt,
        )

        # Check order kwarg, asanyarray allows 1d to pass through
        assert_raises(
            ValueError, np.einsum, 'i->i',
    ...
    def test_einsum_object_errors(self):
        ...
        a = np.array(
            [DestructoBox(i, 5) for i in range(1, 10)],
            dtype='object',
        ).reshape(3, 3)
        ...
        b = np.array(
            [DestructoBox(i, 100) for i in range(0, 27)],
            dtype='object',
        ).reshape(3, 3, 3)
        ...
        b = np.array(
            [DestructoBox(i, 55) for i in range(1, 4)],
            dtype='object',
        )
    ...
    def check_einsum_sums(self, dtype, do_opt=False):
        ...
        for n in range(1, 17):
            ...
            assert_equal(
                np.einsum('i,j', a, b, optimize=do_opt),
                np.outer(a, b),
            )
            assert_equal(
                np.einsum(a, [0], b, [1], optimize=do_opt),
                np.outer(a, b),
            )
        ...
        with suppress_warnings() as sup:
            ...
            for n in range(1, 17):
                ...
                b = np.arange(n, dtype=dtype)
                assert_equal(
                    np.einsum('ij, j', a, b, optimize=do_opt),
                    np.dot(a, b),
                )
                assert_equal(
                    np.einsum(a, [0, 1], b, [1], optimize=do_opt),
                    np.dot(a, b),
                )
            ...
            for n in range(1, 17):
                if n < 8 or dtype != 'f2':
                    ...
                    assert_equal(
                        np.einsum('ij,jk', a, b, optimize=do_opt),
                        np.dot(a, b),
                    )
                    assert_equal(
            ...
            for n in range(1, 17):
                ...
                np.einsum(
                    'ij,jk', a, b, out=c, dtype='f8', casting='unsafe',
                    optimize=do_opt,
                )
                assert_equal(
                    c,
            ...
            if np.dtype(dtype) != np.dtype('f2'):
                ...
                np.einsum(
                    'ijk,jil->kl', a, b, out=c,
                    dtype='f8', casting='unsafe', optimize=do_opt,
                )
                assert_equal(
                    c, np.tensordot(
                        a.astype('f8'), b.astype('f8'),
                        axes=([1, 0], [0, 1]),
                    ).astype(dtype),
                )
                c[...] = 0
                np.einsum(
                    a, [0, 1, 2], b, [1, 0, 3], [2, 3], out=c,
                    dtype='f8', casting='unsafe', optimize=do_opt,
                )
                assert_equal(
                    c, np.tensordot(
        ...
        assert_array_equal(
            np.einsum('ji,i->', x, y, optimize=optimize),
            [2.],
        )  # contig_contig_outstride0_two
        assert_array_equal(
            np.einsum('i,ij->', y, x, optimize=optimize),
            [2.],
        )  # stride0_contig_outstride0_two
        assert_array_equal(
            np.einsum('ij,i->', x, y, optimize=optimize),
            [2.],
    ...
    def test_einsum_broadcast(self):
        ...
        for opt in [True, False]:
            assert_equal(np.einsum('ij...,j...->ij...',
                         A, B, optimize=opt), ref)
            assert_equal(np.einsum('ij...,...j->ij...',
                         A, B, optimize=opt), ref)
            # used to raise error
        ...
        for opt in [True, False]:
            assert_equal(np.einsum('ik...,k...->i...',
                         A, B, optimize=opt), ref)
            assert_equal(np.einsum('ik...,...kj->i...j',
                         A, B, optimize=opt), ref)
            assert_equal(np.einsum('...k,kj', A, B, optimize=opt),
        ...
        for opt in [True, False]:
            ...
            assert_equal(np.einsum('...kl,k', a, v, optimize=opt),
                         ref)  # used to raise error
