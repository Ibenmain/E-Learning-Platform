...
class TestUfuncKwargs:
    ...
    def test_sig_dtype(self):
        ...
        assert_raises(
            TypeError, np.add, 1, 2, signature='ii->i',
            dtype=int,
        )
...
class TestUfunc:
    ...
    def test_use_output_signature_for_all_arguments(self):
        ...
        res = np.power(
            1.5, 2.8, signature=(None, None, np.intp),
            casting='unsafe',
        )
        assert res == 1
        with pytest.raises(TypeError):
            ...
            for x, y in itertools.product([aa, -aa], [bb, -bb]):
                ...
                for tcout in 'efdg':
                    ...
                    if tc in 'FDG':
                        ...
                        assert_raises(TypeError, np.true_divide,
                                      x, y, dtype=dtout)
                    else:
                        tgt = float(x)/float(y)
    ...
    def test_vecdot_matvec_vecmat_complex(self, ufunc, shape1, shape2, conj):
        ...
        if not conj:
            assert_array_equal(
                arr1.reshape(shape1) @ arr2.reshape(shape2),
                expected1,
            )
        ...
        actual3 = ufunc(
            arr1.reshape(shape1).astype('object'),
    ...
    def test_type_cast(self):
        ...
        assert_array_equal(
            np.vecdot(a, a), np.sum(a*a, axis=-1),
            err_msg=msg,
        )
        ...
        assert_array_almost_equal(
            np.vecdot(a, b), np.sum(a*b, axis=-1),
            err_msg=msg,
        )
    ...
    def test_keepdims_argument(self):
        ...
        assert_array_equal(
            c, (a.transpose(1, 0, 2) * b.transpose(0, 2, 1))
            .sum(1, keepdims=True),
        )
    ...
    def test_can_ignore_signature(self):
        ...
        assert_raises(
            ValueError, umt.matrix_multiply, single_vec, single_vec,
            out,
        )
    ...
    def test_object_comparison(self):
        ...
        assert_equal(np.equal(arr1d, arr1d), np.array(
    ...
    def test_safe_casting(self):
        ...
        assert_array_equal(
            assert_no_warnings(np.add, a, 1.1),
            [2.1, 3.1, 4.1],
        )
    ...
    indexed_ufuncs = [
        np.add, np.subtract, np.multiply, np.floor_divide,
    ...
    def test_reduce_arguments(self):
        ...
        assert_equal(
            f(
                d, 0, None, out=None, keepdims=False, initial=0,
                where=True,
            ), r,
        )
        ...
        assert_raises(
            TypeError, f, d, axis='invalid', dtype=None,
            keepdims=True,
        )
        # invalid dtype
        ...
        # invalid mix
        assert_raises(
            TypeError, f, d, 0, keepdims='invalid', dtype='invalid',
            out=None,
        )
        ...
        assert_raises(
            TypeError, f, d, 0, keepdims=True, invalid='invalid',
            out=None,
        )
        assert_raises(
            TypeError, f, d, axis=0, dtype=None, keepdims=True,
            out=None, invalid=0,
        )
        assert_raises(
            TypeError, f, d, axis=0, dtype=None,
            out=None, invalid=0,
        )
    ...
    def test_reducelike_byteorder_resolution(self):
        ...
        assert_array_equal(np.add.accumulate(arr_be),
