...
types = [
    np.bool, np.byte, np.ubyte, np.short, np.ushort, np.intc, np.uintc,
...
class TestTypes:
    def test_types(self):
        for atype in types:
            a = atype(1)
            assert_(a == 1, 'error with {!r}: got {!r}'.format(atype, a))
...
class TestPower:
    def test_small_types(self):
        for t in [np.int8, np.int16, np.float16]:
            a = t(3)
            b = a ** 4
            assert_(b == 81, 'error with {!r}: got {!r}'.format(t, b))

    def test_large_types(self):
        for t in [np.int32, np.int64, np.float32, np.float64, np.longdouble]:
            a = t(51)
            b = a ** 4
            msg = 'error with {!r}: got {!r}'.format(t, b)
...
class TestConversion:
    ...
    def test_numpy_scalar_relational_operators(self):
        ...
        for dt1 in np.typecodes['AllInteger']:
            assert_(1 > np.array(0, dtype=dt1)[
                    ()], 'type {} failed'.format(dt1))
