...
def _get_cancast_table():
    ...
    convert_cast = {
        '.': Casting.unsafe, '~': Casting.same_kind,
...
class TestCasting:
    ...
    def get_data_variation(self, arr1, arr2, aligned=True, contig=True):
        ...
        if aligned:
            new1 = as_strided(
                from_bytes[:-1].view(arr1.dtype),
                arr1.shape, (stride1,),
            )
            new2 = as_strided(
                to_bytes[:-1].view(arr2.dtype),
                arr2.shape, (stride2,),
            )
        else:
            new1 = as_strided(
    ...
    def test_simple_direct_casts(self, from_dt):
        ...
        for to_dt in simple_dtype_instances():
            ...
            if (
                (from_dt.alignment == 1 and to_dt.alignment == 1) or
