...
class TestRegression:
    ...
    def test_method_args(self):
        ...
        funcs1 = [
            'argmax', 'argmin', 'sum', 'any', 'all', 'cumsum',
    ...
    def test_noncommutative_reduce_accumulate(self):
        ...
        assert_array_equal(
            np.subtract.accumulate(tosubtract),
            np.array([0, -1, -3, -6, -10]),
        )
        assert_array_equal(
            np.divide.accumulate(todivide),
            np.array([2., 4., 16.]),
        )
    ...
    def test_string_NULL(self):
        # Changeset 3557
        assert_equal(
            np.array('a\x00\x0b\x0c\x00').item(),
            'a\x00\x0b\x0c',
        )
    ...
    def test_fromstring(self):
        assert_equal(
            np.fromstring('12:09:09', dtype=int, sep=':'),
            [12, 9, 9],
        )
    ...
    def test_sign_for_complex_nan(self):
        ...
        with np.errstate(invalid='ignore'):
            ...
            have = np.sign(C)
            want = np.array([
                -1+0j, -0.6+0.8j, 0+0j, 0.8-0.6j, 1+0j,
    ...
    def test_void_scalar_constructor(self):
        ...
        for proto in range(2, pickle.HIGHEST_PROTOCOL + 1):
            assert_(
                pickle.loads(
                    pickle.dumps(test_string, protocol=proto),
                ) == test_string,
            )
            assert_(
                pickle.loads(
                    pickle.dumps(test_record, protocol=proto),
                ) == test_record,
            )
    ...
    def test_ticket_1539(self):
        dtypes = [
            x for x in np._core.sctypeDict.values()
            if (
    ...
    def test_ticket_1538(self):
        ...
        for name in 'eps epsneg max min resolution tiny'.split():
            assert_equal(
                type(getattr(x, name)), np.float32,
                err_msg=name,
            )
    ...
    def test_squeeze_axis_handling(self):
        ...
        assert_equal(
            np.squeeze(oldsqueeze, axis=None),
            np.array([1, 2, 3]),
        )
    ...
    def test_logspace_base_does_not_determine_dtype(self):
        ...
        out2 = np.logspace(
            start, stop, num=5, axis=1, dtype=np.float32,
            base=np.array([10.0]),
        )
