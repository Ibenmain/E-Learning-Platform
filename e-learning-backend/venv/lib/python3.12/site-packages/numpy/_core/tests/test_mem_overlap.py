...
from numpy._core._multiarray_tests import solve_diophantine
...
def _check_assignment(srcidx, dstidx):
    ...
    assert_(
        np.all(arr == cpy),
        'assigning arr[{}] = arr[{}]'.format(dstidx, srcidx),
    )
...
def check_may_share_memory_exact(a, b):
    ...
    if got != exact:
        err_msg = '    ' + '\n    '.join([
            'base_a - base_b = {!r}'.format(
                a.__array_interface__['data'][0] - b.__array_interface__['data'][0]),
            'shape_a = {!r}'.format(a.shape),
            'shape_b = {!r}'.format(b.shape),
            'strides_a = {!r}'.format(a.strides),
            'strides_b = {!r}'.format(b.strides),
            'size_a = {!r}'.format(a.size),
            'size_b = {!r}'.format(b.size),
        ])
...
def test_internal_overlap_fuzz():
    ...
    while min(overlap, no_overlap) < min_count:
        ...
        strides = tuple(
            rng.randint(-1000, 1000, dtype=np.intp)
            for j in range(ndim)
        )
        shape = tuple(
            rng.randint(1, 30, dtype=np.intp)
            for j in range(ndim)
        )
...
class TestUFunc:
    ...
    def test_binary_ufunc_accumulate_fuzz(self):
        ...
        self.check_unary_fuzz(
            np.add.accumulate, get_out_axis_size,
    ...
    def test_binary_ufunc_reduce_fuzz(self):
        ...
        self.check_unary_fuzz(
            np.add.reduce, get_out_axis_size,
            dtype=np.int16, count=500,
        )
