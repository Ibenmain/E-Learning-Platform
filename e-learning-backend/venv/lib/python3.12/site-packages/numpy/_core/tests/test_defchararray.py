...
class TestMethodsScalarValues:
    ...
    def test_decode(self):
        ...
        assert_equal(
            np.char.decode(bytestring, encoding='cp037'),
            'aAaAaA',
        )
    ...
    def test_encode(self):
        unicode = 'aAaAaA'
        assert_equal(
            np.char.encode(unicode, encoding='cp037'),
    ...
    def test_join(self):
        seps = np.array(['-', '_'])
        assert_array_equal(
            np.char.join(seps, 'hello'),
            ['h-e-l-l-o', 'h_e_l_l_o'],
        )

    def test_partition(self):
        assert_equal(
            np.char.partition('This string', ' '),
            ['This', ' ', 'string'],
        )

    def test_rpartition(self):
        assert_equal(
            np.char.rpartition('This string here', ' '),
            ['This string', ' ', 'here'],
        )

    def test_replace(self):
        assert_equal(
            np.char.replace('Python is good', 'good', 'great'),
            'Python is great',
        )
