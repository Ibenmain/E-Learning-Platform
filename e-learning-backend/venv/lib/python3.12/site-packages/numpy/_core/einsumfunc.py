...
def _parse_einsum_input(operands):
    ...
    for char in output_subscript:
        if output_subscript.count(char) != 1:
            raise ValueError(
                'Output character %s appeared more than once in '
                'the output.' % char,
            )
...
def einsum(*operands, out=None, optimize=False, **kwargs):
    ...
    * Chained array operations, in efficient calculation order,
    ...
    unknown_kwargs = [
        k for (k, v) in kwargs.items() if
        k not in valid_einsum_kwargs
    ]
    if len(unknown_kwargs):
        raise TypeError(
            'Did not understand the following kwargs: %s'
            % unknown_kwargs,
        )

    # Build the contraction list and operand
    operands, contraction_list = einsum_path(
        *operands, optimize=optimize,
        einsum_call=True,
    )
