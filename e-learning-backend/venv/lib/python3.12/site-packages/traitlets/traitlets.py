...
from .utils.descriptions import add_article
from .utils.descriptions import class_of
from .utils.descriptions import describe
from .utils.descriptions import repr_type
from .utils.getargspec import getargspec
from .utils.importstring import import_item
from .utils.sentinel import Sentinel
from .utils.warnings import deprecated_method
from .utils.warnings import should_warn
...
class link:
    ...
    def __init__(self, source: t.Any, target: t.Any, transform: t.Any = None) -> None:
        ...
        self._transform, self._transform_inv = transform if transform else (
            lambda x: x,) * 2
...
class TraitType(BaseDescriptor, t.Generic[G, S]):
    ...
    def get(self, obj: HasTraits, cls: type[t.Any] | None = None) -> G | None:
        ...
        try:
            ...
            raise TraitError(
                'Unexpected error in TraitType: default value not set properly') from e
        else:
            return t.cast(G, value)
    ...
    def get_metadata(self, key: str, default: t.Any = None) -> t.Any:
        ...
        warn('Deprecated in traitlets 4.1, ' +
            ...
            if isinstance(value, TraitType):
                ...
                for c in mro_trait:
                    ...
                    if (
                        ...
                        isinstance(trait.default_value,
                                   (str, int, float, bool)) or none_ok
                    ):
                        cls._static_immutable_initial_values[name] = trait.default_value
    ...
    def validate(self, obj: t.Any, value: t.Any) -> G:
        ...
        if isinstance(value, bytes):
            try:
                ...
                raise TraitError(msg.format(
                    value, self.name, class_of(obj))) from e
        self.error(obj, value)
...
class Enum(TraitType[G, G]):
    ...
    def _info(self, as_rst: bool = False) -> str:
        """Returns a description of the trait."""
        none = ' or %s' % (
            '`None`' if as_rst else 'None') if self.allow_none else ''
...
class FuzzyEnum(Enum[G]):
    ...
    def validate(self, obj: t.Any, value: t.Any) -> G:
        ...
        match_func = (lambda v, c: v in c) if substring_matching else (
            lambda v, c: c.startswith(v))
        value = conv_func(value)  # type:ignore[no-untyped-call]
        choices = self.values or []
        matches = [match_func(value, conv_func(c))
                   for c in choices]  # type:ignore[no-untyped-call]
        if sum(matches) == 1:
...
class List(Container[t.List[T]]):
    ...
    def __init__(
        self,
        trait: t.List[T] | t.Tuple[T] | t.Set[T] | Sentinel | TraitType[T,
                                                                        t.Any] | None = None,
...
class Dict(Instance['dict[K, V]']):
    ...
    def __init__(
        self,
        value_trait: TraitType[t.Any, t.Any] | dict[K,
                                                    V] | Sentinel | None = None,
        ...
        **kwargs: t.Any,
        ...
        if is_trait(value_trait):
            ...
            raise TypeError(
                '`value_trait` must be a Trait or None, got %s' % repr_type(
                    value_trait),
            )
...
class UseEnum(TraitType[t.Any, t.Any]):
    ...
    def __init__(
        ...
        assert issubclass(
            enum_class, enum.Enum), 'REQUIRE: enum.Enum, but was: %r' % enum_class
