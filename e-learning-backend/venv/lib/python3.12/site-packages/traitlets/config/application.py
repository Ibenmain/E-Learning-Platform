...
class Application(SingletonConfigurable):
    ...
    option_description: str | Unicode[str,
                                      str | bytes] = Unicode(option_description)
        ...
        for alias, longname in self.aliases.items():
            try:
                ...
                fhelp_lines = cls.class_get_trait_help(
                    trait, helptext=fhelp).splitlines()
    ...
    def emit_flag_help(self) -> t.Generator[str, None, None]:
        ...
        for flags, (cfg, fhelp) in self.flags.items():
            try:
                ...
                self.log.error(
                    'Failed collecting help-message for flag %r, due to: %s', flags, ex)
                raise
    ...
    def parse_command_line(self, argv: ArgvType = None) -> None:
        ...
        if argv is None:
            argv = self._get_sys_argv(
                check_argcomplete=bool(self.subcommands))[1:]
        ...
        for current in reversed(path):
            ...
            if log:
                log.debug('Looking for %s in %s',
                          basefilename, current or os.getcwd())
