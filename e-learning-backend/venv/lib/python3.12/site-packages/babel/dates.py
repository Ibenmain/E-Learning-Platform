...
def get_next_timezone_transition(zone=None, dt=None):
    ...
    if not hasattr(zone, '_utc_transition_times'):
        raise TypeError(
            'Given timezone does not have UTC transition '
...
def get_timezone_name(
    dt_or_tzinfo=None, width='long', uncommon=False,
...
def format_datetime(
    datetime=None, format='medium', tzinfo=None,
    locale=LC_TIME,
):
...
def format_timedelta(
    delta, granularity='second', threshold=.85,
            ...
            for patterns in _iter_patterns(unit):
                if patterns is not None:
                    pattern = patterns.get(
                        plural_form) or patterns.get('other')
                    break
...
def _format_fallback_interval(start, end, skeleton, tzinfo, locale):
    if skeleton in locale.datetime_skeletons:  # Use the given skeleton
        def format(dt): return format_skeleton(
            skeleton, dt, tzinfo, locale=locale)
    elif all((isinstance(d, date) and not isinstance(d, datetime)) for d in (start, end)):  # Both are just dates
        def format(dt): return format_date(dt, locale=locale)
    elif all((isinstance(d, time) and not isinstance(d, date)) for d in (start, end)):  # Both are times
        def format(dt): return format_time(dt, tzinfo=tzinfo, locale=locale)
    else:
        def format(dt): return format_datetime(
        ...
        for rule in rules:
            ...
            if 'after' in rule:
                raise NotImplementedError(
                    "'after' is deprecated as of CLDR 29.")
...
class DateTimePattern:
    ...
    def __repr__(self):
        return '<{} {!r}>'.format(type(self).__name__, self.pattern)
...
class DateTimeFormat:
    ...
    def extract(self, char):
        ...
        if char == 'y':
            ...
            raise NotImplementedError(
                'Not implemented: extracting {!r} from {!r}'.format(char, self.value))
    ...
    def format_period(self, char, num):
        ...
        raise ValueError(
            'Could not format period {} in {}'.format(period, self.locale))
        ...
        # Included additional elif condition to add support for 'Xx' in timezone format
            ...
            if num == 1:
                return get_timezone_gmt(
                    self.value, width='iso8601_short', locale=self.locale,
                    return_z=True,
                )
            elif num in (2, 4):
                return get_timezone_gmt(
                    self.value, width='short', locale=self.locale,
                    return_z=True,
                )
            elif num in (3, 5):
                return get_timezone_gmt(
                    self.value, width='iso8601', locale=self.locale,
                    return_z=True,
                )
        elif char == 'x':
            if num == 1:
    ...
    def get_week_number(self, day_of_period, day_of_week=None):
        ...
        first_day = (
            day_of_week - self.locale.first_week_day -
            day_of_period + 1
        ) % 7
        if first_day < 0:
            first_day += 7
        ...
        if self.locale.first_week_day == 0:
            ...
            max_weeks = date(year=self.value.year, day=28,
                             month=12).isocalendar()[1]
...
def match_skeleton(skeleton, options, allow_different_fields=False):
    ...
    get_input_field_width = dict(
        t[1] for t in tokenize_pattern(skeleton) if t[0] == 'field').get
    best_skeleton = None
    best_distance = None
    for option in options:
        get_opt_field_width = dict(
            t[1] for t in tokenize_pattern(option) if t[0] == 'field').get
        distance = 0
