...
class PoFileParser:
    ...
    def _add_message(self):
            ...
            for idx, translation in self.translations:
                if idx >= self.catalog.num_plurals:
                    self._invalid_pofile(
                        '', self.offset, 'msg has more translations than num_plurals of catalog')
                    continue
        ...
        if self.context:
            ...
            msgctxt = None
        message = Message(
            msgid, string, list(self.locations), set(self.flags),
    ...
    def _process_string_continuation_line(self, line, lineno):
        if self.in_msgid:
            ...
            self._invalid_pofile(
                line, lineno, "Got line starting with \" but not in msgid, msgstr or msgctxt")
            return
        s.append(line)
...
def write_po(
    fileobj, catalog, width=76, no_location=False, omit_header=False,
    ...
    for message in _sort_messages(catalog, sort_by=sort_by):
        ...
        if not no_location:
            ...
            for filename, lineno in locations:
                if lineno and include_lineno:
                    location = '%s:%d' % (
                        filename.replace(os.sep, '/'), lineno)
                else:
                    location = '%s' % filename.replace(os.sep, '/')
        ...
        if message.previous_id and include_previous:
            _write_comment(
                'msgid %s' % _normalize(message.previous_id[0]),
                prefix='|',
            )
            if len(message.previous_id) > 1:
                _write_comment(
                    'msgid_plural %s' % _normalize(
