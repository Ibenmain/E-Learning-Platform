...
def check_and_call_extract_file(
    filepath, method_map, options_map,
...
def extract_from_file(
    method, filename, keywords=DEFAULT_KEYWORDS,
    ...
    with open(filename, 'rb') as fileobj:
        return list(
            extract(
                method, fileobj, keywords, comment_tags,
...
def extract(
    method, fileobj, keywords=DEFAULT_KEYWORDS, comment_tags=(),
    ...
    results = func(
        fileobj, keywords.keys(), comment_tags,
        options=options or {},
    )
...
def extract_python(fileobj, keywords, comment_tags, options):
    ...
    def next_line(): return fileobj.readline().decode(encoding)
    ...
    for tok, value, (lineno, _), _, _ in tokens:
        if call_stack == -1 and tok == NAME and value in ('def', 'class'):
            ...
            if (tok == OP and value == ')') or nested:
                ...
                yield (
                    message_lineno, funcname, messages,
...
def extract_javascript(fileobj, keywords, comment_tags, options):
    ...
    for token in tokenize(
        ...
        if (  # Turn keyword`foo` expressions into keyword("foo") calls:
            ...
            # we've seen nothing after the keyword...
