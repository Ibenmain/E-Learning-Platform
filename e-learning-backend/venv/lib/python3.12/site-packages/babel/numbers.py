...
def get_territory_currencies(
    territory, start_date=None, end_date=None,
...
def _format_currency_long_name(
    ...
    # Step 3.
    unit_pattern = get_currency_unit_pattern(
        currency, count=number_n, locale=locale)
...
class NumberPattern:
    ...
    def __init__(
        self, pattern, prefix, suffix, grouping,
    ...
    def apply(
        ...
        if force_frac:
            ...
            warnings.warn(
                'The force_frac parameter to NumberPattern.apply() is deprecated.', DeprecationWarning)
        ...
        if not decimal_quantization or (self.exp_prec and frac_prec == (0, 0)):
            frac_prec = (frac_prec[0], max(
                [frac_prec[1], get_decimal_precision(value)]))

        # Render scientific notation.
        if self.exp_prec:
            number = ''.join([
                self._quantize_value(
                    value, locale, frac_prec, group_separator),
