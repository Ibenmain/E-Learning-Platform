...
class PluralRule:
    ...
    def __repr__(self):
        ...
        return '<{} {!r}>'.format(
            type(self).__name__,
            ', '.join([
                '{}: {}'.format(tag, rules[tag]) for tag in _plural_tags
                if tag in rules
            ]),
        )
    ...
    tags = property(
        lambda x: frozenset(i[0] for i in x.abstract), doc="""
...
def to_javascript(rule):
    ...
    for tag, ast in PluralRule.parse(rule).abstract:
        result.append('{} ? {!r} : '.format(to_js(ast), tag))
...
def to_python(rule):
    ...
    for tag, ast in PluralRule.parse(rule).abstract:
        ...
        result.append(' if ({}): return {!r}'.format(
            to_python_func(ast), str(tag)))
...
_RULES = [
    ...
    ('word', re.compile(
        fr'\b(and|or|is|(?:with)?in|not|mod|[{"".join(_VARS)}])\b')),
...
def tokenize_rule(s):
    ...
    while pos < end:
        for tok, rule in _RULES:
            ...
            raise RuleError(
                'malformed CLDR pluralization rule.  '
...
class _Parser:
    ...
    def expect(self, type_, value=None, term=None):
        ...
        raise RuleError('expected {} but got {!r}'.format(
            term, self.tokens[-1][1]))
...
class _Compiler:
    ...
    def compile_n(x): return 'n'
    def compile_i(x): return 'i'
    def compile_v(x): return 'v'
    def compile_w(x): return 'w'
    def compile_f(x): return 'f'
    def compile_t(x): return 't'
    def compile_c(x): return 'c'
    def compile_e(x): return 'e'
    def compile_value(x, v): return str(v)
...
class _PythonCompiler(_Compiler):
    ...
    def compile_relation(self, method, expr, range_list):
        ...
        return '{}({}, {})'.format(
            method.upper(), self.compile(expr),
            compile_range_list,
        )
...
class _JavaScriptCompiler(_GettextCompiler):
    ...
    def compile_i(x): return 'parseInt(n, 10)'
    compile_v = compile_zero
    compile_w = compile_zero
    compile_f = compile_zero
    compile_t = compile_zero
    ...
    def compile_relation(self, method, expr, range_list):
        ...
        if method == 'in':
            expr = self.compile(expr)
            code = '(parseInt({}, 10) == {} && {})'.format(expr, expr, code)
        return code
