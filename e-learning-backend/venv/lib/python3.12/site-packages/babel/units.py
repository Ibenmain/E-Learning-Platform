...
class UnknownUnitError(ValueError):
    def __init__(self, unit, locale):
        ValueError.__init__(
            self, '{} is not a known unit in {}'.format(unit, locale))
...
def format_unit(value, measurement_unit, length='long', format=None, locale=LC_NUMERIC):
    ...
    return '{} {}'.format(  # pragma: no cover
        formatted_value,
        (get_unit_name(measurement_unit, length=length,
         locale=locale) or measurement_unit),
    )
...
def _find_compound_unit(numerator_unit, denominator_unit, locale=LC_NUMERIC):
    ...
    return _find_unit_pattern('{}-per-{}'.format(bare_numerator_unit, bare_denominator_unit), locale=locale)
...
def format_compound_unit(
    ...
    if isinstance(denominator_value, str):  # Denominator is preformatted
        ...
        if denominator_value == 1:  # support perUnitPatterns when the denominator is 1
            denominator_unit = _find_unit_pattern(
                denominator_unit, locale=locale)
    ...
    per_pattern = locale._data['compound_unit_patterns'].get(
        'per', {}).get(length, {}).get('compound', '{0}/{1}')
