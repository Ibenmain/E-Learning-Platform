...
long = int
xrange = range
def bytechr(num): return bytes([num])
...
class CTypesData:
    ...
    def _convert_to_address(self, BClass):
        if BClass is None:
            raise TypeError(
                'cannot convert {!r} to an address'.format(
                    self._get_c_name(),
                ),
            )
        else:
    ...
    def __iter__(self):
        raise TypeError(
            'cdata %r does not support iteration' % (
                self._get_c_name()
            ),
        )
...
class CTypesGenericPtr(CTypesData):
    ...
    def _convert_to_address(self, BClass):
        if (
            BClass in (self.__class__, None) or BClass._automatic_casts
...
class CTypesBackend:
    ...
    def new_pointer_type(self, BItem):
        ...
        #
        if (
            BItem is self.ffi._get_cached_btype(model.void_type) or
            ...
            def _initialize(blob, init):
                if isinstance(init, bytes):
                    ...
                    if isinstance(init, CTypesGenericArray):
                        if (
                            len(init) != len(blob) or
                            not isinstance(init, CTypesArray)
                        ):
                            raise TypeError(
                                'length/type mismatch: {}'.format(init))
                    init = tuple(init)
    ...
    def complete_struct_or_union(
        self, CTypesStructOrUnion, fields, tp,
        ...
        for fname, BField, bitsize in fields:
            ...
            if bitsize < 0:
                def getter(
                    self, fname=fname, BField=BField,
                    offset=CTypesStructOrUnion._offsetof(fname),
                    PTR=ctypes.POINTER(BField._ctype),
                ):
                ...
                if issubclass(BField, CTypesGenericArray):
                    ...
                    if BField._declared_length == 0:
                        def getter(
                            self, fname=fname, BFieldPtr=BField._CTPtr,
                            offset=CTypesStructOrUnion._offsetof(fname),
                            PTR=ctypes.POINTER(BField._ctype),
                        ):
                            addr = ctypes.addressof(self._blob)
