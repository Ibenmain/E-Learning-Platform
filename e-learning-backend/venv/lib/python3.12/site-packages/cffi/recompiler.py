...
USE_LIMITED_API = (
    sys.platform != 'win32' or sys.version_info < (3, 0) or
...
class GlobalExpr:
    ...
    def as_python_expr(self):
        return "b'%s%s',%d" % (
            self.type_op.as_python_bytes(), self.name,
            self.check_value,
        )
...
class StructUnionExpr:
    def __init__(
        self, name, type_index, flags, size, alignment, comment,
...
class Recompiler:
    ...
    def write_c_source_to_f(self, f, preamble):
        ...
        if self.ffi._embedding is not None:
            ...
            prnt(
                '# define _CFFI_PYTHON_STARTUP_FUNC  PyInit_{}'.format(
                    base_module_name,
                ),
            )
            prnt('#else')
            prnt(
                '# define _CFFI_PYTHON_STARTUP_FUNC  init{}'.format(
                    base_module_name,
                ),
            )
            prnt('#endif')
    ...
    def _convert_funcarg_to_c(self, tp, fromvar, tovar, errcode):
        ...
        #
            ...
            isinstance(tp, model.StructOrUnionOrEnum) or
        ...
        #
        self._prnt('  {} = {}({}{});'.format(
            tovar, converter, fromvar, extraarg))
        self._prnt(
    ...
    def _generate_cpy_typedef_ctx(self, tp, name):
        ...
        if getattr(tp, 'origin', None) == 'unknown_type':
            ...
            self._struct_ctx(
                tp.totype, tp.totype.name, approxname=tp.name,
                named_ptr=tp,
            )
    ...
    def _generate_cpy_function_decl(self, tp, name):
        ...
        if tp.abi:
            ...
            abi = ''
        name_and_arguments = '{}_cffi_d_{}({})'.format(
            abi, name, repr_arguments)
        ...
        def need_indirection(type):
            return (
                isinstance(type, model.StructOrUnion) or
                (
        ...
        if difference:
            repr_arguments = ', '.join(arguments)
            repr_arguments = repr_arguments or 'void'
            name_and_arguments = '{}_cffi_f_{}({})'.format(
                abi, name,
                repr_arguments,
            )
    ...
    def _struct_decl(self, tp, cname, approxname):
        ...
        for fname, ftype, fbitsize, fqual in self._enum_fields(tp):
            try:
                ...
                while (
                    isinstance(ftype, model.ArrayType)
    ...
    def _struct_ctx(self, tp, cname, approxname, named_ptr=None):
        ...
        if (
            tp not in self.ffi._parser._included_declarations and
            (
        ...
        if reason_for_not_expanding is None:
            ...
            for fldname, fldtype, fbitsize, fqual in enumfields:
                ...
                if cname is None or fbitsize >= 0:
                    ...
                    offset = 'offsetof({}, {})'.format(
                        tp.get_c_name(''), fldname)
                c_fields.append(
                    FieldExpr(
                        fldname, offset, size, fbitsize,
            ...
            if cname is None:  # unknown name, for _add_missing_struct_unions
                ...
                if named_ptr is not None:
                    ...
                    align = 'offsetof(struct _cffi_align_{}, y)'.format(
                        approxname)
                comment = None
        else:
            size = '(size_t)-1'
        ...
        self._lsts['struct_union'].append(
            StructUnionExpr(
                tp.name, type_index, flags, size, align, comment,
                first_field_index, c_fields,
            ),
        )
    ...
    def _generate_cpy_const(
        self, is_int, name, tp=None, category='const',
        check_value=None,
    ):
    ...
    def _enum_ctx(self, tp, cname):
        ...
        for enumerator, enumvalue in zip(tp.enumerators, tp.enumvalues):
            self._lsts['global'].append(
                GlobalExpr(
                    enumerator, '_cffi_const_%s' % enumerator, type_op,
    ...
    def _generate_cpy_macro_ctx(self, tp, name):
        ...
        self._lsts['global'].append(
            GlobalExpr(
                name, '_cffi_const_%s' % name, type_op,
    ...
    def _extern_python_decl(self, tp, name, tag_and_space):
        ...
        def may_need_128_bits(tp):
            return (
                isinstance(tp, model.PrimitiveType) and
                tp.name == 'long double'
            )
        #
...
def make_c_source(ffi, module_name, preamble, target_c_file, verbose=False):
    ...
    return _make_c_or_py_source(
        ffi, module_name, preamble, target_c_file,
        verbose,
    )
...
def make_py_source(ffi, module_name, target_py_file, verbose=False):
    return _make_c_or_py_source(
        ffi, module_name, None, target_py_file,
        verbose,
    )
...
def _patch_for_embedding(patchlist):
    if sys.platform == 'win32':
        ...
        if MSVCCompiler is not None:
            _patch_meth(
                patchlist, MSVCCompiler, '_remove_visual_c_ref',
...
def _patch_for_target(patchlist, target):
    ...
    if target.endswith('.*'):
        ...
        if sys.platform == 'win32':
            ...
            target += '.so'
    _patch_meth(
        patchlist, build_ext, 'get_ext_filename',
        lambda self, ext_name: target,
    )


def recompile(
    ffi, module_name, preamble, tmpdir='.', call_c_compiler=True,
    ...
    if preamble is not None:
        ...
        updated = make_c_source(
            ffi, module_name, preamble, c_file,
