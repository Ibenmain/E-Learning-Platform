...
def add_cffi_module(dist, mod_spec):
    ...
    if not os.path.exists(build_file_name):
        ...
        error('{!r} does not name an existing file{}'.format(
            build_file_name, ext))
    ...
    try:
        ...
        error(
            '{!r}: object {!r} not found in module'.format(
                mod_spec,
                ffi_var_name,
            ),
        )
...
def _set_py_limited_api(Extension, kwds):
    ...
    if (
        'py_limited_api' not in kwds and not hasattr(sys, 'gettotalrefcount')
