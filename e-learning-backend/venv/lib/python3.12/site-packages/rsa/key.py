...
class AbstractKey:
    ...
    @classmethod
    def _load_pkcs1_pem(cls: type[T], keyfile: bytes) -> T:
    ...
    @classmethod
    def _load_pkcs1_der(cls: type[T], keyfile: bytes) -> T:
        """Loads a key in PKCS#1 PEM format, implement in a subclass.
    ...
    @classmethod
    def load_pkcs1(cls: type[T], keyfile: bytes, format: str = 'PEM') -> T:
    ...
    def _assert_format_exists(
        ...
        try:
            ...
            raise ValueError(
                'Unsupported format: {!r}, try one of {}'.format(
                    file_format, formats),
            ) from ex
    ...
    def _update_blinding_factor(self) -> tuple[int, int]:
        ...
        with self.mutex:
            if self.blindfac < 0:
                ...
                self.blindfac_inverse = rsa.common.inverse(
                    self.blindfac, self.n)
            else:
...
class PrivateKey(AbstractKey):
    ...
    def _load_pkcs1_der(cls, keyfile: bytes) -> PrivateKey:
        ...
        if priv[0] != 0:
            raise ValueError(
                'Unable to read this file, version %s != 0' % priv[0])
...
def newkeys(
    ...
    (p, q, e, d) = gen_keys(nbits, getprime_func,
                            accurate=accurate, exponent=exponent)
