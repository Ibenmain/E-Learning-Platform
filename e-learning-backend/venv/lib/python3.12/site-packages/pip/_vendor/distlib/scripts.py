...
from .util import get_export_entry
...
class ScriptMaker:
    ...
    def _get_shebang(self, encoding, post_interp=b'', options=None):
        ...
        if self.executable:
            ...
            executable = os.path.join(sysconfig.get_path(
                'scripts'), 'python%s' % sysconfig.get_config_var('EXE'))
            ...
            if os.name == 'nt':
                ...
                executable = os.path.join(
                    sysconfig.get_config_var('BINDIR'),
                    'python{}{}'.format(sysconfig.get_config_var(
                        'VERSION'), sysconfig.get_config_var('EXE')),
                )
        if options:
        ...
        if (
            sys.platform == 'cli' and '-X:Frames' not in post_interp and
    ...
    def _get_script_text(self, entry):
        return self.script_template % dict(
            module=entry.prefix, import_name=entry.suffix.split('.')[
                0], func=entry.suffix,
        )
        ...
        for name in names:
            ...
            if use_launcher:  # pragma: no cover
                ...
                try:
                    ...
                    self._fileop.write_binary_file(outname, script_bytes)
                    logger.debug(
                        'Able to replace executable using '
                        '.deleteme logic',
                    )
                    try:
                        os.remove(dfname)
    ...
    def get_script_filenames(self, name):
        ...
        if 'X.Y' in self.variants:
            result.add('{}{}{}.{}'.format(name, self.variant_separator,
    ...
    def _copy_script(self, script, filenames):
        ...
        if not adjust:
            ...
            logger.info('copying and adjusting %s -> %s',
                        script, self.target_dir)
    ...
    if os.name == 'nt' or (os.name == 'java' and os._name == 'nt'):  # pragma: no cover
        ...
        def _get_launcher(self, kind):
            ...
            if name not in WRAPPERS:
                msg = (
                    'Unable to find resource %s in package %s' %
