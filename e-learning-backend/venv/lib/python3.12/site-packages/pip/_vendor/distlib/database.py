...
from .metadata import LEGACY_METADATA_FILENAME
from .metadata import Metadata
from .metadata import METADATA_FILENAME
from .metadata import WHEEL_METADATA_FILENAME
from .util import cached_property
from .util import CSVReader
from .util import CSVWriter
from .util import parse_name_and_version
from .util import parse_requirement
from .util import read_exports
from .util import write_exports
...
DIST_FILES = ('INSTALLER', METADATA_FILENAME, 'RECORD',
              'REQUESTED', 'RESOURCES', EXPORTS_FILENAME, 'SHARED')
            ...
            for entry in rset:
                ...
                try:
                    if self._include_dist and entry.endswith(DISTINFO_EXT):
                        possible_filenames = [
                            METADATA_FILENAME, WHEEL_METADATA_FILENAME, LEGACY_METADATA_FILENAME]
...
class Distribution:
    ...
    def _get_requirements(self, req_attr):
        ...
        logger.debug('%s: got requirements %r from metadata: %r',
                     self.name, req_attr, reqts)
...
class InstalledDistribution(BaseInstalledDistribution):
    ...
    def __repr__(self):
        return '<InstalledDistribution {!r} {} at {!r}>'.format(self.name, self.version, self.path)
...
class EggInfoDistribution(BaseInstalledDistribution):
    ...
    def _get_metadata(self, path):
        ...
        if path.endswith('.egg'):
            if os.path.isdir(path):
                ...
                fileobj = StringIO(zipf.get_data(
                    'EGG-INFO/PKG-INFO').decode('utf8'))
    ...
    def __repr__(self):
        return '<EggInfoDistribution {!r} {} at {!r}>'.format(self.name, self.version, self.path)
    ...
    def to_dot(self, f, skip_disconnected=True):
        ...
        for dist, adjs in self.adjacency_list.items():
            ...
            for other, label in adjs:
                if label is not None:
                    f.write(
                        '"{}" -> "{}" [label="{}"]\n'.format(dist.name, other.name, label))
                else:
