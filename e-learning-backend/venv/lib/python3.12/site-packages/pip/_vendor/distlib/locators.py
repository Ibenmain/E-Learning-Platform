...
from .compat import (
    urljoin, urlparse, urlunparse, url2pathname, pathname2url, queue, quote, unescape, build_opener,
    HTTPRedirectHandler as BaseRedirectHandler, text_type, Request, HTTPError, URLError,
)
from .database import Distribution, DistributionPath, make_dist
from .metadata import Metadata, MetadataInvalidError
from .util import (
    cached_property, ensure_slash, split_filename, get_project_data, parse_requirement,
    ...
    def _should_queue(self, link, referrer, rel):
        ...
        if path.endswith(self.source_extensions + self.binary_extensions + self.excluded_extensions):
            ...
            if host.lower() == 'localhost':
                ...
                result = True
        logger.debug('should_queue: %s (%s) from %s -> %s',
                     link, rel, referrer, result)
        return result
        ...
        for root, dirs, files in os.walk(self.base_dir):
            for fn in files:
                if self.should_include(fn, root):
                    ...
                    url = urlunparse(('file', '', pathname2url(
                        os.path.abspath(fn)), '', '', ''))
...
class DependencyFinder:
    ...
    def try_to_replace(self, provider, other, problems):
        ...
        if unmatched:
            ...
            problems.add(('cantreplace', provider,
                         other, frozenset(unmatched)))
            result = False
        else:
