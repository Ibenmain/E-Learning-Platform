...
from collections.abc import Iterator, Mapping, MutableSequence, Iterable
import zipfile
import zipimport
import warnings
import stat
...
def get_supported_platform():
    ...
    if m is not None and sys.platform == 'darwin':
        try:
            plat = 'macosx-{}-{}'.format('.'.join(_macos_vers()
                                         [:2]), m.group(3))
        except ValueError:
            # not macOS
            pass
...
class Distribution:
    ...
    def load_entry_point(self, group: str, name: str) -> _ResolvedEntryPoint:
        ...
        if ep is None:
            raise ImportError(
                'Entry point {!r} not found'.format((group, name)))
        return ep.load()

    @overload
    def get_entry_map(
        self, group: None = None) -> dict[str, dict[str, EntryPoint]]: ...
