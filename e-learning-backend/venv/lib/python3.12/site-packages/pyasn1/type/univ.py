        ...
        try:
            ...
            try:
                ...
                raise error.PyAsn1Error(
                    'Can\'t coerce {!r} into integer: {}'.format(value, exc),
                )
...
class BitString(base.SimpleAsn1Type):
    ...
    def __init__(self, value=noValue, **kwargs):
        ...
        if value is noValue:
            if self.defaultBinValue is not noValue:
                value = self.fromBinaryString(
                    self.defaultBinValue, internalFormat=True)

            elif self.defaultHexValue is not noValue:
                value = self.fromHexString(
                    self.defaultHexValue, internalFormat=True)
    ...
    def fromHexString(cls, value, internalFormat=False, prepend=None):
        ...
        try:
            ...
            raise error.PyAsn1Error(
                '{}.fromHexString() error: {}'.format(cls.__name__, exc))
    ...
    def fromBinaryString(cls, value, internalFormat=False, prepend=None):
        ...
        try:
            ...
            raise error.PyAsn1Error(
                '{}.fromBinaryString() error: {}'.format(cls.__name__, exc))
    ...
    def fromOctetString(cls, value, internalFormat=False, prepend=None, padding=0):
        ...
        """
        value = SizedInteger(int.from_bytes(
            bytes(value), 'big') >> padding).setBitLength(len(value) * 8 - padding)
        ...
        if isinstance(value, SizedInteger):
            ...
            # "'1011'B" -- ASN.1 schema representation (deprecated)
                ...
                try:
                    ...
                    raise error.PyAsn1Error(
                        'unknown bit name(s) in {!r}'.format(names))
...
class Null(OctetString):
    ...
    subtypeSpec = OctetString.subtypeSpec + \
        constraint.SingleValueConstraint(b'')
...
class ObjectIdentifier(base.SimpleAsn1Type):
    ...
    def prettyIn(self, value):
        ...
        try:
            tupleOfInts = tuple([int(subOid)
                                for subOid in value if subOid >= 0])
        ...
        raise error.PyAsn1Error('Malformed Object ID {} at {}'.format(
    ...
    def prettyIn(self, value):
        if isinstance(value, RelativeOID):
            ...
            if '-' in value:
                raise error.PyAsn1Error(
                    # sys.exc_info in case prettyIn was called while handling an exception
                    'Malformed RELATIVE-OID {} at {}: {}'.format(
                        value, self.__class__.__name__, sys.exc_info()[1]),
                )
            try:
                return tuple([int(subOid) for subOid in value.split('.') if subOid])
        ...
        raise error.PyAsn1Error(
            'Malformed RELATIVE-OID {} at {}'.format(value, self.__class__.__name__))
...
class Real(base.SimpleAsn1Type):
    ...
    def prettyIn(self, value):
        if isinstance(value, tuple) and len(value) == 3:
            if (
                not isinstance(value[0], (int, float)) or
            ...
            if componentValue is not noValue:
                if isinstance(componentValue, base.ConstructedAsn1Type):
                    myClone.setComponentByPosition(
                        idx, componentValue.clone(
                            cloneValueFlag=cloneValueFlag),
                    )
                else:
    ...
    def setComponentByPosition(
        ...
        if value is noValue:
            ...
            if not subtypeChecker(
                value, verifyConstraints and matchTags,
...
class SequenceAndSetBase(base.ConstructedAsn1Type):
        ...
        if value is noValue:
            if componentTypeLen:
                ...
                if isinstance(value, base.ConstructedAsn1Type):
                    value = value.clone(
                        cloneValueFlag=componentType[idx].isDefaulted)
            ...
            if componentTypeLen:
                ...
                if isinstance(subComponentType, base.SimpleAsn1Type):
                    ...
                    raise error.PyAsn1Error(
                        '%s can cast only scalar values' % componentType.__class__.__name__)
            ...
            (verifyConstraints or matchTags or matchConstraints) and
            componentTypeLen
        ):
            subComponentType = componentType.getTypeByPosition(idx)
            if subComponentType is not noValue:
                ...
                if not subtypeChecker(
                    ...
                    if not componentType[idx].openType:
                        raise error.PyAsn1Error(
                            'Component value is tag-incompatible: {!r} vs {!r}'.format(value, componentType))
    ...
    def prettyPrintType(self, scope=0):
        ...
        for idx, componentType in enumerate(self.componentType.values() or self._componentValues):
            representation += ' ' * scope
            if self.componentType:
                representation += '"%s"' % self.componentType.getNameByPosition(
                    idx)
            else:
                representation += '"%s"' % self._dynamicNames.getNameByPosition(
                    idx)
            representation = '{} = {}\n'.format(
