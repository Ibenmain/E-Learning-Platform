...
class NamedType:
    ...
    def __repr__(self):
        representation = '{}={!r}'.format(self.name, self.asn1Object)
    ...
    *namedTypes: :class:`~pyasn1.type.namedtype.NamedType`
    ...
    def __init__(self, *namedTypes, **kwargs):
        ...
        self.__hasOptionalOrDefault = any([
            True for namedType in self.__namedTypes
        ...
        self.__requiredComponents = frozenset(
            [idx for idx, nt in enumerate(
                self.__namedTypes) if not nt.isOptional and not nt.isDefaulted],
        )
        self.__keys = frozenset(
            [namedType.name for namedType in self.__namedTypes])
        self.__values = tuple(
            [namedType.asn1Object for namedType in self.__namedTypes])
        self.__items = tuple([(namedType.name, namedType.asn1Object)
                             for namedType in self.__namedTypes])

    def __repr__(self):
        representation = ', '.join(['%r' % x for x in self.__namedTypes])
        return '<{} object, types {}>'.format(
            self.__class__.__name__, representation,
        )
        ...
        for idx, namedType in enumerate(self.__namedTypes):
            ...
            for _tagSet in tagMap.presentTypes:
                if _tagSet in tagToPosMap:
                    return NamedTypes.PostponedError('Duplicate component tag {} at {}'.format(_tagSet, namedType))
    ...
    def __computeNameToPosMap(self):
        ...
        for idx, namedType in enumerate(self.__namedTypes):
            if namedType.name in nameToPosMap:
                return NamedTypes.PostponedError('Duplicate component name {} at {}'.format(namedType.name, namedType))
        ...
        for namedType in self.__namedTypes:
            ...
            for tagSet in tagMap:
                if unique and tagSet in presentTypes:
                    return NamedTypes.PostponedError('Non-unique tagSet {} of {} at {}'.format(tagSet, namedType, self))
            ...
            if defaultType is None:
                ...
                return NamedTypes.PostponedError('Duplicate default ASN.1 type at {}'.format(self))
