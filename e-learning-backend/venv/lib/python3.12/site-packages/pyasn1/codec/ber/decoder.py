...
class AbstractPayloadDecoder:
    ...
    def valueDecoder(
        ...
        **options,
        ...
        """
        raise error.PyAsn1Error('SingleItemDecoder not implemented for {}'.format(
            tagSet))  # TODO: Seems more like an NotImplementedError?
    ...
    def indefLenValueDecoder(
        ...
        **options,
        ...
        The decoder is allowed to consume as many bytes as necessary.
        """
        raise error.PyAsn1Error('Indefinite length mode decoder not implemented for {}'.format(
            tagSet))  # TODO: Seems more like an NotImplementedError?
...
class NullPayloadDecoder(AbstractSimplePayloadDecoder):
    ...
    def valueDecoder(
        ...
        **options,
        ...
        if chunk:
            raise error.PyAsn1Error(
                'Unexpected %d-octet substrate for Null' % length)
        ...
        while index < substrateLen:
            ...
            if subId < 128:
                ...
                while nextSubId >= 128:
                    ...
                    if index >= substrateLen:
                        raise error.SubstrateUnderrunError(
                            'Short substrate for sub-OID past {}'.format(
                                reloid),
                        )
                    nextSubId = chunk[index]
                    index += 1
    ...
    def _decodeComponentsSchemaless(
        ...
        while length == -1 or substrate.tell() < original_position + length:
            ...
            asn1Object = protoComponent.clone(
                ...
                tagSet=tag.TagSet(
                    protoComponent.tagSet.baseTag, *tagSet.superTags),
            )
    ...
    def valueDecoder(
        ...
        **options,
        ...
        if asn1Spec.typeId in (univ.Sequence.typeId, univ.Set.typeId):
            ...
            if LOG:
                LOG(
                    'decoding {}deterministic {} type {!r} chosen by type ID'.format(
                ...
                if not namedTypes:
                    ...
                    try:
                        if isDeterministic:
                            ...
                            componentType = namedTypes.getTagMapNearPosition(
                                idx)
                        ...
                        raise error.PyAsn1Error(
                            'Excessive components decoded at {!r}'.format(
                                asn1Spec),
                        )
                ...
                if namedTypes.hasOpenTypes:
                    ...
                    if openTypes or options.get('decodeOpenTypes', False):
                        ...
                        for idx, namedType in enumerate(namedTypes.namedTypes):
                            ...
                            containerValue = asn1Object.getComponentByPosition(
                                idx)
...
class ChoicePayloadDecoder(ConstructedPayloadDecoderBase):
    ...
    def valueDecoder(
        ...
        **options,
        ...
        if asn1Object.tagSet == tagSet:
            if LOG:
                LOG('decoding {} as explicitly tagged CHOICE'.format(tagSet))
            ...
            if LOG:
                LOG('decoding {} as untagged CHOICE'.format(tagSet))
        ...
        if LOG:
            LOG('decoded component {}, effective tag set {}'.format(
                component, effectiveTagSet))
    ...
    def indefLenValueDecoder(
        ...
        **options,
        ...
        if LOG:
            LOG(
                'decoding {} as {}tagged CHOICE'.format(
            ...
            if LOG:
                for chunk in peekIntoStream(substrate, length):
                    if isinstance(chunk, SubstrateUnderrunError):
                        yield chunk
                LOG(
                    'decoding as untagged ANY, substrate '
    ...
    def indefLenValueDecoder(
        ...
        **options,
        ...
        if isTagged:
            ...
            if LOG:
                LOG('decoding as untagged ANY, header substrate %s' %
                    debug.hexdump(chunk))
        ...
        if substrateFun and substrateFun is not self.substrateCollector:
            ...
            for chunk in substrateFun(
                    asn1Object, chunk + substrate, length +
                len(chunk), options,
            ):
                yield chunk
...
TAG_MAP = {
    ...
    # conflicts with SequenceOf
    univ.Sequence.tagSet: SequenceOrSequenceOfPayloadDecoder(),
        ...
        **options,
        ...
        while state is not stStop:
            ...
            if state is stGetValueDecoderByTag:
                ...
                if LOG:
                    LOG('codec {} chosen by a built-in type, decoding {}'.format(
                        concreteDecoder and concreteDecoder.__class__.__name__ or '<none>', state is stDecodeValue and 'value' or 'as explicit tag'))
                    debug.scope.push(
            ...
            if state is stGetValueDecoderByAsn1Spec:
                ...
                if asn1Spec.__class__ is tagmap.TagMap:
                    ...
                    if LOG:
                        ...
                        for firstOctet, v in asn1Spec.presentTypes.items():
                            LOG('  {} -> {}'.format(firstOctet, v.__class__.__name__))

                        if asn1Spec.skipTypes:
                            LOG('but neither of: ')
                            for firstOctet, v in asn1Spec.skipTypes.items():
                                LOG('  {} -> {}'.format(firstOctet,
                                    v.__class__.__name__))
                        LOG('new candidate ASN.1 spec is {}, chosen by {}'.format(
                            chosenSpec is None and '<none>' or chosenSpec.prettyPrintType(), tagSet))

                elif tagSet == asn1Spec.tagSet or tagSet in asn1Spec.tagMap:
                    chosenSpec = asn1Spec
                    if LOG:
                        LOG('candidate ASN.1 spec is %s' %
                ...
                if chosenSpec is not None:
                    try:
                        ...
                        if LOG:
                            LOG('value decoder chosen for an ambiguous type by type ID {}'.format(
                                chosenSpec.typeId))
                        ...
                        try:
                            ...
                            if LOG:
                                LOG('value decoder chosen by base {}'.format(
                                    baseTagSet))
                ...
                if LOG:
                    LOG('codec {} chosen by ASN.1 spec, decoding {}'.format(
                        state is stDecodeValue and concreteDecoder.__class__.__name__ or '<none>', state is stDecodeValue and 'value' or 'as explicit tag'))
                    debug.scope.push(
            ...
            if state is stDecodeValue:
                ...
                if length == -1:  # indef length
                    ...
                    if not substrateFun and bytesRead != length:
                        raise PyAsn1Error(
                            'Read {} bytes instead of expected {}.'.format(
                                bytesRead, length),
                        )
                        ...
                        raise PyAsn1Error(
                            'Read {} bytes are more than expected {}.'.format(
                                bytesRead, length),
                        )

                if LOG:
                    LOG(
                        'codec {} yields type {}, value:\n{}\n...'.format(
                            concreteDecoder.__class__.__name__, value.__class__.__name__,
                            isinstance(
                                value, base.Asn1Item) and value.prettyPrint() or value,
                        ),
                    )
            ...
            if state is stTryAsExplicitTag:
                ...
                if LOG:
                    LOG('codec {} chosen, decoding {}'.format(
                        concreteDecoder and concreteDecoder.__class__.__name__ or '<none>', state is stDecodeValue and 'value' or 'as failure'))

            if state is stDumpRawValue:
                concreteDecoder = self.defaultRawDecoder

                if LOG:
                    LOG('codec %s chosen, decoding value' %
            ...
            if state is stErrorCondition:
                raise error.PyAsn1Error(
                    '{} not in asn1Spec: {!r}'.format(tagSet, asn1Spec),
                )
...
class Decoder:
        ...
        if 'substrateFun' in options:
            ...
            def substrateFunWrapper(asn1Object, substrate, length, options=None):
                ...
                try:
                    ...
                    # nosec assert_used
                    assert isinstance(substrate, io.BytesIO)
    ...
    def _callSubstrateFunV4asV5(substrateFunV4, asn1Object, substrate, length):
        ...
        value, nextSubstrate = substrateFunV4(
            asn1Object, substrate_bytes, length)
