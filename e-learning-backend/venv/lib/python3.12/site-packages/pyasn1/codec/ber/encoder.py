...
class AbstractItemEncoder:
    ...
    def encode(self, value, asn1Spec=None, encodeFun=None, **options):
        ...
        for idx, singleTag in enumerate(tagSet.superTags):
            ...
            if LOG:
                LOG(
                    'encoded {} octets (tag + payload) into {}'.format(
    ...
    def encodeValue(self, value, asn1Spec, encodeFun, **options):
        ...
        if 0 <= second <= 39:
            if first == 1:
                ...
                raise error.PyAsn1Error(
                    'Impossible first/second arcs at {}'.format(value))
        ...
        for subOid in oid:
            if 0 <= subOid <= 127:
                ...
                raise error.PyAsn1Error(
                    'Negative OID arc {} at {}'.format(subOid, value))
    ...
    def encodeValue(self, value, asn1Spec, encodeFun, **options):
        ...
        for subOid in value.asTuple():
            if 0 <= subOid <= 127:
                ...
                raise error.PyAsn1Error(
                    'Negative RELATIVE-OID arc {} at {}'.format(subOid, value))
    ...
    def _chooseEncBase(self, value):
        ...
        for i in range(3):
            (
                ...
                encBase[i],
                exponent[i],
            ) = self._dropFloatingPoint(mantissa[i], encBase[i], exponent[i])
            ...
            for idx, component in enumerate(value.values()):
                if namedTypes:
                    ...
                    if namedType.isOptional and not component.isValue:
                        if LOG:
                            LOG('not encoding OPTIONAL component {!r}'.format(
                                namedType))
                        continue

                    if namedType.isDefaulted and component == namedType.asn1Object:
                        if LOG:
                            LOG('not encoding DEFAULT component {!r}'.format(
                                namedType))
                        continue
                ...
                if namedTypes and namedType.openType:
                    ...
                    if wrapType.typeId in (
                        ...
                        if wrapType.isSameTypeWith(component):
                            ...
                            if LOG:
                                LOG('wrapped with wrap type {!r}'.format(
                                    wrapType))
...
class ChoiceEncoder(AbstractItemEncoder):
    def encodeValue(self, value, asn1Spec, encodeFun, **options):
        if asn1Spec is None:
            ...
            names = [
                namedType.name for namedType in asn1Spec.componentType.namedTypes
                if namedType.name in value
            ]
            if len(names) != 1:
                raise error.PyAsn1Error('{} components for Choice at {!r}'.format(
    ...
    def __call__(self, value, asn1Spec=None, **options):
        ...
        try:
            ...
            try:
                ...
                raise error.PyAsn1Error(
                    'No encoder for {!r} ({})'.format(value, tagSet))
