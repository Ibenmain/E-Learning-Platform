...
from .base import catch_errors_and_unavailability
...
class MetadataEndpoint(BaseEndpoint):
   ...
   """
    ...
    def create_metadata_response(
        self, uri, http_method='GET', body=None,
        headers=None,
    ):
    ...
    def validate_metadata(self, array, key, is_required=False, is_list=False, is_url=False, is_issuer=False):
        ...
        if key not in array:
            if is_required:
                raise ValueError(f'key {key} is a mandatory metadata.')

        elif is_issuer:
            if not utils.is_secure_transport(array[key]):
                raise ValueError(
                    f'key {key}: {array[key]} must be an HTTPS URL')
            if '?' in array[key] or '&' in array[key] or '#' in array[key]:
                raise ValueError(
                    f'key {key}: {array[key]} must not contain query or fragment components')

        elif is_url:
            if not array[key].startswith('http'):
                raise ValueError(f'key {key}: {array[key]} must be an URL')

        elif is_list:
            if not isinstance(array[key], list):
                raise ValueError(f'key {key}: {array[key]} must be an Array')
            for elem in array[key]:
                if not isinstance(elem, str):
                    raise ValueError(
                        f'array {key}: {array[key]} must contains only string (not {elem})')
    ...
    def validate_metadata_token(self, claims, endpoint):
        ...
        self.validate_metadata(claims, 'token_endpoint',
                               is_required=True, is_url=True)
    ...
    def validate_metadata_revocation(self, claims, endpoint):
        ...
        self.validate_metadata(
            claims, 'revocation_endpoint_auth_signing_alg_values_supported', is_list=True)
        self.validate_metadata(claims, 'revocation_endpoint',
                               is_required=True, is_url=True)
    ...
    def validate_metadata_introspection(self, claims, endpoint):
        ...
        self.validate_metadata(
            claims, 'introspection_endpoint_auth_signing_alg_values_supported', is_list=True)
        self.validate_metadata(
