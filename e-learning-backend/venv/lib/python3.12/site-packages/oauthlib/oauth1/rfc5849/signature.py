...
def collect_parameters(
    uri_query='', body=None, headers=None,
    ...
    if headers:
        ...
        if authorization_header is not None:
            params.extend([
                i for i in utils.parse_authorization_header(
...
def _verify_hmac(
    ...
    sig_base_str = signature_base_string(
        request.http_method, bs_uri,
        norm_params,
    )
    signature = _sign_hmac(
        hash_algorithm_name, sig_base_str,
...
def sign_hmac_sha1(base_string, client_secret, resource_owner_secret):
    ...
    """
    warnings.warn(
        'use sign_hmac_sha1_with_client instead of sign_hmac_sha1',
        DeprecationWarning,
    )
...
def sign_hmac_sha512_with_client(
    sig_base_str: str,
    client,
):
    return _sign_hmac(
        'SHA-512', sig_base_str,
        client.client_secret, client.resource_owner_secret,
    )
...
def _verify_rsa(
    ...
    try:
        ...
        if not verify_ok:
            log.debug(
                'Verify failed: RSA with ' + alg.hash_alg.name +
...
def sign_rsa_sha1(base_string, rsa_private_key):
    ...
    """
    warnings.warn(
        'use _sign_rsa("SHA-1", ...) instead of sign_rsa_sha1',
        DeprecationWarning,
    )
