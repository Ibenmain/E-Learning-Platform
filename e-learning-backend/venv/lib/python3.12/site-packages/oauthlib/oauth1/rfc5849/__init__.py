...
class Client:
    ...
    def __repr__(self):
        ...
        attribute_str = ', '.join(f'{k}={v}' for k, v in attrs.items())
    ...
    def get_oauth_signature(self, request):
        ...
        log.debug(f'Collected params: {collected_params}')

        normalized_params = signature.normalize_parameters(collected_params)
        normalized_uri = signature.base_string_uri(
            uri, headers.get('Host', None))
        log.debug(f'Normalized params: {normalized_params}')
        log.debug(f'Normalized URI: {normalized_uri}')
        ...
        log.debug(f'Signing: signature base string: {base_string}')
    ...
    def get_oauth_params(self, request):
        ...
        if request.body is not None and content_type_eligible:
            params.append(('oauth_body_hash', base64.b64encode(
                hashlib.sha1(request.body.encode('utf-8')).digest()).decode('utf-8')))
