...
from ..utils import partial_pipeline_data
from ..utils import sanitize_redirect
from ..utils import slugify
from ..utils import user_is_active
...
class SanitizeRedirectTest(unittest.TestCase):
    ...
    def test_invalid_redirect(self):
        self.assertEqual(sanitize_redirect(
            ['myapp.com'], {'foo': 'bar'}), None)

    def test_wrong_path_redirect(self):
        self.assertEqual(
            sanitize_redirect(
                ['myapp.com'], 'http://notmyapp.com/path/'), None,
        )
...
class BuildAbsoluteURITest(unittest.TestCase):
    ...
    def test_path_http(self):
        self.assertEqual(
            build_absolute_uri(
                self.host, 'http://barfoo.com'), 'http://barfoo.com',
        )

    def test_path_https(self):
        self.assertEqual(
            build_absolute_uri(
                self.host, 'https://barfoo.com'), 'https://barfoo.com',
        )

    def test_host_ends_with_slash_and_path_starts_with_slash(self):
        self.assertEqual(
            build_absolute_uri(
                self.host + '/', '/foo/bar'), 'http://foobar.com/foo/bar',
        )
...
class PartialPipelineData(unittest.TestCase):
    def test_returns_partial_when_uid_and_email_do_match(self):
        ...
        key, val = ('foo', 'bar')
        partial = partial_pipeline_data(
            backend, None, *(), **dict([(key, val)]))
