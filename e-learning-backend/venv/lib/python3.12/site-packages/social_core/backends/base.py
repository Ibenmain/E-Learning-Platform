...
from ..utils import SSLHttpAdapter
...
class BaseAuth:
        ...
        for entry in extra_data_entries:
            ...
            if size >= 1 and size <= 3:
                if size == 3:
                    ...
                    discard = False
                value = response.get(
                    name, details.get(name, details.get(alias)))
    ...
    def auth_allowed(self, response, details):
        ...
        emails = [email.lower()
                  for email in self.setting('WHITELISTED_EMAILS', [])]
        domains = [domain.lower()
                   for domain in self.setting('WHITELISTED_DOMAINS', [])]
    ...
    def request(self, url, method='GET', *args, **kwargs):
        ...
        kwargs.setdefault(
            'timeout',
            self.setting('REQUESTS_TIMEOUT') or self.setting(
                'URLOPEN_TIMEOUT'),
        )
        if self.SEND_USER_AGENT and 'User-Agent' not in kwargs['headers']:
            kwargs['headers']['User-Agent'] = self.setting(
