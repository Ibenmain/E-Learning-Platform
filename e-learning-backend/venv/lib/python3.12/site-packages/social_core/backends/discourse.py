...
class DiscourseAuth(BaseAuth):
    ...
    def auth_url(self):
        ...
        base_64_payload = urlsafe_b64encode(
            payload.encode('utf8')).decode('ascii')
    ...
    def auth_complete(self, *args, **kwargs):
        ...
        param_signature = hmac.new(
            self.setting('SECRET').encode(
                'utf8'), sso_params.encode('utf8'), sha256,
        ).hexdigest()
        ...
        decoded_params = urlsafe_b64decode(
            sso_params.encode('utf8')).decode('ascii')
        ...
        kwargs.update(
            {'sso': '', 'sig': '', 'backend': self, 'response': response})
