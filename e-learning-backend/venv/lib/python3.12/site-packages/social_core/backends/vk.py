...
class VKontakteOpenAPI(BaseAuth):
    ...
    def auth_complete(self, *args, **kwargs):
        ...
        kwargs.update(
            {'backend': self, 'response': self.user_data(mapping['mid'])})
...
class VKAppOAuth2(VKOAuth2):
    ...
    def auth_complete(self, *args, **kwargs):
        required_params = ('is_app_user', 'viewer_id',
                           'access_token', 'api_id')
        ...
        if auth_key:
            check_key = md5(
                '_'.join([key, self.data.get('viewer_id'), secret]
                         ).encode('utf-8'),
            ).hexdigest()
            if check_key != auth_key:
                raise ValueError(
                    'VK.com authentication failed: invalid ' 'auth key')
...
def vk_api(backend, method, data):
    ...
    if 'access_token' not in data:
        ...
        data['sig'] = md5(
            (''.join(param_list) + secret).encode('utf-8')).hexdigest()
    else:
