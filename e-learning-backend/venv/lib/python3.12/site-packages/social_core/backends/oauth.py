...
from ..exceptions import AuthStateForbidden
from ..exceptions import AuthStateMissing
from ..exceptions import AuthTokenError
from ..exceptions import AuthUnknownError
...
class OAuthAuth(BaseAuth):
    ...
    def get_request_state(self):
        request_state = self.data.get(
            'state') or self.data.get('redirect_state')
    ...
    def revoke_token(self, token, uid):
        if self.REVOKE_TOKEN_URL:
            ...
            data = urlencode(
                params) if self.REVOKE_TOKEN_METHOD != 'GET' else None
...
class BaseOAuth1(OAuthAuth):
    ...
    def unauthorized_token(self):
        ...
        response = self.request(
            ...
            params=params,
            auth=OAuth1(
                key, secret, callback_uri=self.get_redirect_uri(state)),
...
class BaseOAuth2(OAuthAuth):
    ...
    def extra_data(self, user, uid, response, details=None, *args, **kwargs):
        ...
        data['token_type'] = response.get(
            'token_type') or kwargs.get('token_type')
        return data
    ...
    def process_error(self, data):
        if data.get('error'):
            ...
            raise AuthFailed(self, data.get(
                'error_description') or data['error'])
        elif 'denied' in data:
