...
from allauth.socialaccount.providers.facebook.constants import GRAPH_API_VERSION
from allauth.socialaccount.providers.facebook.constants import NONCE_LENGTH
from allauth.socialaccount.providers.facebook.constants import NONCE_SESSION_KEY
from allauth.socialaccount.providers.facebook.constants import PROVIDER_ID
...
class FacebookProvider(OAuth2Provider):
    ...
    def get_login_url(self, request, **kwargs):
        ...
        if method == 'js_sdk':
            ...
            action = "'%s'" % escapejs(kwargs.get(
                'action') or AuthAction.AUTHENTICATE)
            scope = "'%s'" % escapejs(kwargs.get('scope', ''))
            js = 'allauth.facebook.login({}, {}, {}, {})'.format(
                next,
                action,
                process,
                scope,
            )
            ret = 'javascript:{}'.format(quote(js))
        elif method == 'oauth2':
