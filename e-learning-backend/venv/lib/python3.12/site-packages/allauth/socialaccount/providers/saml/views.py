...
from .utils import build_auth
from .utils import build_saml_config
...
class ACSView(SAMLViewMixin, View):
    def dispatch(self, request, organization_slug):
        ...
        acs_session = LoginSession(
            request, 'saml_acs_session', 'saml-acs-session')
        acs_session.store.update(
            {'request': httpkit.serialize_request(request)})
...
class FinishACSView(SAMLViewMixin, View):
    def dispatch(self, request, organization_slug):
        ...
        if acs_request_data:
            acs_request = httpkit.deserialize_request(
                acs_request_data, HttpRequest())
        acs_session.delete()
        if not acs_request:
