...
class OAuth2Provider(Provider):
    ...
    def get_pkce_params(self) -> dict:
        ...
        if enabled is None:
            ...
            enabled = settings.get('OAUTH_PKCE_ENABLED',
                                   self.pkce_enabled_default)
        if enabled:
    ...
    def get_oauth2_adapter(self, request):
        if not hasattr(self, 'oauth2_adapter_class'):
            raise ImproperlyConfigured(
                f"No oauth2_adapter_class set for {self!r}")
    ...
    def get_redirect_from_request_kwargs(self, request):
        ...
        kwargs['auth_params'] = self.get_auth_params_from_request(
            request, action)
        return kwargs
