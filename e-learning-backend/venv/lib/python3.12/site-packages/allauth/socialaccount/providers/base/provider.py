...
class Provider:
    ...
    def get_redirect_from_request_kwargs(self, request):
        ...
        kwargs['process'] = get_request_param(
            request, 'process', AuthProcess.LOGIN)
        return kwargs
    ...
    def cleanup_email_addresses(
        ...
        if email and email.lower() not in [a.email.lower() for a in addresses]:
            addresses.insert(
                0,
                EmailAddress(email=email, verified=bool(
                    email_verified), primary=True),
            )
        ...
        addresses.sort(key=lambda a: (
            a.primary, a.verified, a.email), reverse=True)
