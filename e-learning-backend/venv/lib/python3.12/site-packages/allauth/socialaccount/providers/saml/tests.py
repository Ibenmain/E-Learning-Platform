...
@pytest.mark.parametrize(
    ...
    [
        ...
        (True, {'reject_idp_initiated_sso': False},
         {}, '/set-by-idp', '/set-by-idp'),
        (
            True,
...
def test_acs(
    ...
    finish_url = reverse('saml_finish_acs', kwargs={
    ...
    if idp_initiated and advanced.get('reject_idp_initiated_sso', True):
        ...
        assert account.extra_data['Role'] == [
            'view-profile', 'manage-account-links']
        email = EmailAddress.objects.get(user=account.user)
        assert email.email == (
            user.email if is_connect else 'john.doe@email.org')
...
def test_login_on_get(client, db, saml_settings):
    resp = client.get(reverse('saml_login', kwargs={
                      'organization_slug': 'org'}))
...
def test_metadata(
    ...
    resp = client.get(reverse('saml_metadata', kwargs={
                      'organization_slug': 'org'}))
    assert resp.status_code == 200
...
def test_build_saml_config_without_metadata_url(rf, provider_config):
    ...
    assert config['idp']['singleSignOnService'] == {
        'url': 'https://idp.org/sso/'}
    assert config['idp']['singleLogoutService'] == {
        'url': 'https://idp.saml.org/slo/'}
...
def test_build_saml_config(rf, provider_config):
    ...
    metadata_url = request.build_absolute_uri(
        reverse('saml_metadata', args=['org']))
