...
@pytest.mark.parametrize(
    ...
    [
        ...
        ('https://abs.org/foo?key={key}',
         {'key': ' '}, 'https://abs.org/foo?key=+'),
    ],
)
...
def test_deserialize_request(rf):
    ...
    assert not httpkit.deserialize_request(
        serialized, HttpRequest()).is_secure()
    data = json.loads(serialized)
    data['scheme'] = 'https'
    assert httpkit.deserialize_request(
        json.dumps(data), HttpRequest()).is_secure()
