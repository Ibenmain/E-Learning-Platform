...
def get_server() -> Fido2Server:
    ...
    if app_settings.WEBAUTHN_ALLOW_INSECURE_ORIGIN:
        def verify_origin(o): return True  # noqa
...
def get_credentials(user) -> list[AttestedCredentialData]:
    ...
    for authenticator in authenticators:
        ...
        if credential_data:
            credentials.append(
                authenticator.wrap().authenticator_data.credential_data)
    return credentials
...
def extract_user_from_response(response: dict):
    try:
        ...
        user_pk = url_str_to_user_pk(
            websafe_decode(user_handle).decode('utf8'))
...
def complete_authentication(user, response: dict) -> Authenticator:
    ...
    clear_state()
    authenticator = get_authenticator_by_credential_id(
        user, binding.credential_id)
