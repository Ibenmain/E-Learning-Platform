...
def test_reauthenticate(
    ...
    resp = auth_client.get(headless_reverse(
        'headless:mfa:manage_recovery_codes'))
    ...
    with webauthn_authentication_bypass(passkey):
        resp = auth_client.get(headless_reverse(
            'headless:mfa:reauthenticate_webauthn'))
        data = resp.json()
...
def test_2fa_login(
    ...
    with webauthn_authentication_bypass(passkey) as credential:
        resp = client.get(headless_reverse(
            'headless:mfa:authenticate_webauthn'))
        assert 'request_options' in resp.json()['data']
        resp = client.post(
