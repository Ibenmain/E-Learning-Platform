...
def get_email_verification_url(request: HttpRequest, emailconfirmation) -> str:
    ...
    url = get_frontend_url(
        request, 'account_confirm_email', key=emailconfirmation.key)
    if not url:
...
def login_on_verification(request, verification) -> HttpResponse | None:
...
def send_verification_email(request, user, signup=False, email=None) -> bool:
    ...
    if not email:
        email_address = (
            EmailAddress.objects.filter(user=user).order_by(
                'verified', 'pk').first()
        )
        if email_address:
            email = email_address.email
