...
from allauth.account.models import EmailConfirmationMixin
...
class EmailVerificationModel(EmailConfirmationMixin):
    def __init__(self, email_address: EmailAddress, key: str | None = None):
...
def get_pending_verification(
...
) -> tuple[EmailVerificationModel | None, dict[str, Any] | None]:
    if peek:
    ...
    if user_id_str := data.get('user_id'):
        user_id = get_user_model()._meta.pk.to_python(
            user_id_str)  # type: ignore[union-attr]
