...
def test_user_with_mfa_only(
    ...
    if not allauth_settings.MFA_ENABLED and with_totp:
        return
    user = user_factory(with_totp=with_totp,
                        password=None if with_password else '!')
    ...
    assert {m['url'] for m in methods} == set(
...
def test_reauthentication(settings, auth_client, user_password):
    ...
    assert methods[-1] == {'method': 'password',
                           'at': ANY, 'reauthenticated': True}
