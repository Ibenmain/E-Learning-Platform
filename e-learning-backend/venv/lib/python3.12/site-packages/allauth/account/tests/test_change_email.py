...
def test_add_with_two_limiter(auth_client, user, settings):
    ...
    resp = auth_client.post(
        reverse('account_email'), {'action_add': '',
...
def test_add_not_allowed(
    ...
    if prevent_enumeration:
        ...
        assertTemplateUsed(
            resp, 'account/messages/email_confirmation_failed.txt')
        assert resp.status_code == 302
...
@pytest.mark.parametrize(
    ...
    [
        (AuthenticationMethod.EMAIL, 'pri@ma.il',
         ['sec@ma.il'], 'pri@ma.il', False),
        (AuthenticationMethod.EMAIL, 'pri@ma.il',
         ['sec@ma.il'], 'sec@ma.il', True),
        (AuthenticationMethod.EMAIL, 'pri@ma.il', [], 'pri@ma.il', False),
        (AuthenticationMethod.USERNAME, 'pri@ma.il',
         ['sec@ma.il'], 'pri@ma.il', False),
        (AuthenticationMethod.USERNAME, 'pri@ma.il',
         ['sec@ma.il'], 'sec@ma.il', True),
...
def test_remove_email(
    ...
    assert EmailAddress.objects.filter(
        email=delete_email).exists() == (not success)
    if not success:
        assertTemplateUsed(
            resp, 'account/messages/cannot_delete_primary_email.txt')
