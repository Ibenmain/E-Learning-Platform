...
def test_signup_prevent_enumeration(
    ...
    assert resp.status_code == 200
    resp = client.post(
        reverse('account_email_verification_sent'), data={'code': ''})
    assert resp.status_code == 200
    assert resp.context['form'].errors == {'code': ['This field is required.']}
    resp = client.post(
        reverse('account_email_verification_sent'), data={'code': '123'})
    assert resp.status_code == 200
    assert resp.context['form'].errors == {'code': ['Incorrect code.']}
    # Max attempts
    resp = client.post(
        reverse('account_email_verification_sent'), data={'code': '456'})
    assert resp.status_code == 302
...
def test_add_or_change_email(
    ...
    assert EmailAddress.objects.filter(
        user=user).count() == (1 if change_email else 2)
