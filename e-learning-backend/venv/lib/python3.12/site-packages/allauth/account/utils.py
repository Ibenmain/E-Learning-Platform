...
def user_email(user, *args, commit=False):
    if args and args[0]:
        args = [args[0].lower()]
    ret = user_field(user, app_settings.USER_MODEL_EMAIL_FIELD,
                     *args, commit=commit)
    if ret:
        ret = ret.lower()
    return ret
...
def setup_user_email(request, user, addresses):
    ...
    if email:
        priority_addresses.append(
            EmailAddress(user=user, email=email.lower(),
...
def filter_users_by_email(
    ...
    User = get_user_model()
    email = email.lower()
    mails = list(EmailAddress.objects.filter(
        email=email).select_related('user'))
    is_verified = False
    if prefer_verified:
