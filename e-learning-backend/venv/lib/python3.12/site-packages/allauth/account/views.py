...
class ConfirmEmailView(NextRedirectMixin, LogoutFunctionalityMixin, TemplateView):
    ...
    def get_ajax_data(self):
        ...
        if self.object:
            ...
            ret['user'] = {'display': user_display(
                self.object.email_address.user)}
        return ret
    ...
    def get_context_data(self, **kwargs):
        ...
        emails = list(
            EmailAddress.objects.filter(
                user=self.request.user).order_by('email'),
        )
        ret.update(
            {
...
class PasswordResetFromKeyView(
    ...
    def dispatch(self, request, uidb36, key, **kwargs):
        ...
        if self.key == self.reset_url_key or is_ajax:
            if not is_ajax:
                self.key = self.request.session.get(
                    INTERNAL_RESET_SESSION_KEY, '')
            # (Ab)using forms here to be able to handle errors in XHR #890
            token_form = user_token_form_class(
    ...
    def form_valid(self, form):
        form.save()
        flows.password_reset.finalize_password_reset(
...
@method_decorator(
    login_stage_required(
        stage=LoginByCodeStage.key, redirect_urlname=(
