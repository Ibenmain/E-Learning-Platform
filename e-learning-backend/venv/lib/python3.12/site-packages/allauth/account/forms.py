...
from .utils import setup_user_email
from .utils import sync_user_email_addresses
from .utils import url_str_to_user_pk
    ...
    def __init__(self, *args, **kwargs):
        ...
        if app_settings.AUTHENTICATION_METHOD == AuthenticationMethod.EMAIL:
            ...
            login_field = forms.EmailField(
                label=_('Email'), widget=login_widget)
            ...
            login_widget = forms.TextInput(
                attrs={'placeholder': _(
                    'Username or email'), 'autocomplete': 'email'},
            )
            login_field = forms.CharField(
    ...
    def clean(self):
        ...
        if user:
            ...
            raise adapter.validation_error(
                '%s_password_mismatch' % auth_method.value)
...
class BaseSignupForm(_base_signup_form_class()):  # type: ignore[misc]
    ...
    def save(self, request):
        ...
        setup_user_email(
            request, user, [EmailAddress(email=email)] if email else [])
        return user
...
class ChangePasswordForm(PasswordVerificationMixin, UserForm):
    ...
    def save(self):
        flows.password_change.change_password(
            self.user, self.cleaned_data['password1'])
