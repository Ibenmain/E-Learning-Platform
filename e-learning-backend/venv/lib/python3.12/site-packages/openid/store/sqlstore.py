...
class SQLStore(OpenIDStore):
    ...
    def __init__(self, conn, associations_table=None, nonces_table=None):
        ...
        if (
            hasattr(self.conn, 'IntegrityError') and
            hasattr(self.conn, 'OperationalError')
        ):
            self.exceptions = self.conn

        if not (
            hasattr(self.exceptions, 'IntegrityError') and
    ...
    def __getattr__(self, attr):
        ...
        if attr[:3] == 'db_':
            ...
            raise AttributeError('Attribute {!r} not found'.format(attr))
...
class SQLiteStore(SQLStore):
    ...
    def useNonce(self, *args, **kwargs):
        ...
        try:
            return super().useNonce(*args, **kwargs)
...
class PostgreSQLStore(SQLStore):
    ...
    def db_set_assoc(
        self, server_url, handle, secret, issued, lifetime,
        assoc_type,
    ):
        """
        ...
        if len(rows):
            ...
            return self.db_update_assoc(
                secret, issued, lifetime, assoc_type,
                server_url, handle,
            )
        else:
