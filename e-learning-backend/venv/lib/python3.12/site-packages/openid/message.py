...
def registerNamespaceAlias(namespace_uri, alias):
    ...
    if namespace_uri in list(registered_aliases.values()):
        raise NamespaceAliasRegistrationError(
            'Namespace uri {!r} already registered'.format(namespace_uri),
        )
...
class Message:
    ...
    def toFormMarkup(
        ...
        return str(
            ElementTree.tostring(form, encoding='utf-8'),
            encoding='utf-8',
        )
...
class NamespaceMap:
    ...
    def addAlias(self, namespace_uri, desired_alias, implicit=False):
        ...
        if (
            ...
            msg = fmt % (
                namespace_uri, desired_alias, current_namespace_uri,
                desired_alias,
            )
            raise KeyError(msg)
        ...
        assert (
            desired_alias == NULL_NAMESPACE or
            type(desired_alias) in [str, str]
        ), repr(desired_alias)
