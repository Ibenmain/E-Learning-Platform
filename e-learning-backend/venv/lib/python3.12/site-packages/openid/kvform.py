...
def seqToKV(seq, strict=False):
    ...
    def err(msg):
        formatted = 'seqToKV warning: {}: {!r}'.format(msg, seq)
        if strict:
    ...
    for k, v in seq:
        ...
        if '\n' in k:
            raise KVFormError(
                'Invalid input for seqToKV: key contains newline: {!r}'.format(
                    k),
            )

        if ':' in k:
            raise KVFormError(
                'Invalid input for seqToKV: key contains colon: {!r}'.format(
                    k),
            )

        if k.strip() != k:
            err('Key has whitespace at beginning or end: {!r}'.format(k))

        if isinstance(v, bytes):
            v = v.decode('utf-8')
        elif not isinstance(v, str):
            err('Converting value to string: {!r}'.format(v))
            v = str(v)
        ...
        if v.strip() != v:
            err('Value has whitespace at beginning or end: {!r}'.format(v))
...
def kvToSeq(data, strict=False):
    ...
    def err(msg):
        formatted = 'kvToSeq warning: {}: {!r}'.format(msg, data)
        if strict:
            raise KVFormError(formatted)
        else:
            logger.warning(formatted)
