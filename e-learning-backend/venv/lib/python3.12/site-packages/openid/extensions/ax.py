...
def checkAlias(alias):
    ...
    """
    if ',' in alias:
        raise AXError('Alias {!r} must not contain comma'.format(alias))
    if '.' in alias:
        raise AXError('Alias {!r} must not contain period'.format(alias))
    ...
    def _checkMode(self, ax_args):
        ...
        if mode != self.mode:
            if not mode:
                ...
                raise AXError(
                    'Expected mode {!r}; got {!r}'.format(self.mode, mode))
...
class AXKeyValueMessage(AXMessage):
    ...
    def getSingle(self, type_uri, default=None):
        ...
        if not values:
            ...
            raise AXError(
                'More than one value present for {!r}'.format(type_uri))
...
class FetchResponse(AXKeyValueMessage):
    ...
    def getExtensionArgs(self):
        ...
        update_url = (
            (self.request and self.request.update_url) or
            self.update_url
        )
    ...
    def parseExtensionArgs(self, ax_args):
        ...
        super().parseExtensionArgs(ax_args)
        self.update_url = ax_args.get('update_url')
