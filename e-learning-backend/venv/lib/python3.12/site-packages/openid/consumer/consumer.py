...
from openid.association import default_negotiator
...
from openid.message import OPENID1_NS
...
class Consumer:
    ...
    def complete(self, query, current_url):
        ...
        if (
            response.status in ['success', 'cancel'] and
            response.identity_url is not None
        ):

            disco = Discovery(
                self.session, response.identity_url,
...
class PlainTextConsumerSession:
...
class TypeURIMismatch(ProtocolError):
    ...
    def __str__(self):
        s = '<{}.{}: Required type {} not found in {} for endpoint {}>'.format(
...
class GenericConsumer:
    ...
    def _completeInvalid(self, message, endpoint, _):
        mode = message.getArg(OPENID_NS, 'mode', '<No mode set>')
        return FailureResponse(endpoint, 'Invalid openid.mode: {!r}'.format(mode))
    ...
    def _checkReturnTo(self, message, return_to):
        ...
        try:
            ...
            logger.exception('Verifying return_to arguments: {}'.format(why))
            return False
    ...
    def _idResCheckNonce(self, message, endpoint):
        ...
        try:
            ...
            raise ProtocolError('Malformed nonce: {}'.format(why))
    ...
    def _idResCheckForFields(self, message):
        ...
        for field in require_fields[message.getOpenIDNamespace()]:
            if not message.hasKey(OPENID_NS, field):
                raise ProtocolError(
                    'Missing required field {!r}'.format(field))
        ...
        for field in require_sigs[message.getOpenIDNamespace()]:
            ...
            if message.hasKey(OPENID_NS, field) and field not in signed_list:
                raise ProtocolError('"{}" not signed'.format(field))
    ...
    def _verifyReturnToArgs(query):
        ...
        for pair in bare_args.items():
            if pair not in parsed_args:
                raise ProtocolError(
                    'Parameter %s not in return_to URL' %
                    (pair[0],),
                )
    ...
    def _discoverAndVerify(self, claimed_id, to_match_endpoints):
        ...
        """
        logger.info('Performing discovery on {}'.format(claimed_id))
    ...
    def _createCheckAuthRequest(self, message):
        ...
        if signed:
            ...
            for k in signed.split(','):
                ...
                if val is None:
                    logger.info('Missing signed field {!r}'.format(k))
                    return None
    ...
    def _processCheckAuthResponse(self, response, server_url):
        ...
        if invalidate_handle is not None:
            logger.info(
                'Received "invalidate_handle" from server %s' %
                (server_url,),
            )
            if self.store is None:
                logger.error(
                    'Unexpectedly got invalidate_handle without '
                    'a store!',
                )
            else:
    ...
    def _requestAssociation(self, endpoint, assoc_type, session_type):
        ...
        try:
            ...
            logger.exception('openid.associate request failed: {}'.format(why))
            return None
    ...
    def _createAssociateRequest(self, endpoint, assoc_type, session_type):
        ...
        if (
            not endpoint.compatibilityMode() or
    ...
    def _extractAssociation(self, assoc_response, assoc_session):
        ...
        expires_in_str = assoc_response.getArg(
            OPENID_NS, 'expires_in',
            no_default,
        )
        try:
            expires_in = int(expires_in_str)
        except ValueError as why:
            raise ProtocolError('Invalid expires_in field: {}'.format(why))
        ...
        return Association.fromExpiresIn(
            expires_in, assoc_handle, secret,
            assoc_type,
        )
...
class AuthRequest:
    ...
    def getMessage(self, realm, return_to=None, immediate=False):
        ...
        if self.assoc:
            ...
            assoc_log_msg = 'with association {}'.format(self.assoc.handle)
        else:
