    ...
    def fromBasicServiceEndpoint(cls, endpoint):
        ...
        if type_uris and endpoint.uri is not None:
            ...
            openid_endpoint.parseService(
                endpoint.yadis_url, endpoint.uri,
                endpoint.type_uris,
...
def normalizeURL(url):
    ...
    try:
        ...
        raise DiscoveryFailure('Normalizing identifier: {}'.format(why), None)
    else:
...
def discoverXRI(iname):
    ...
    try:
        ...
        if canonicalID is None:
            raise XRDSError('No CanonicalID found for XRI {!r}'.format(iname))
