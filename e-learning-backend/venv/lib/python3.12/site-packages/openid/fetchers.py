...
USER_AGENT = 'python-openid/{} ({})'.format(openid.__version__, sys.platform)
...
class Urllib2Fetcher(HTTPFetcher):
    ...
    def fetch(self, url, body=None, headers=None):
        if not _allowedURL(url):
            raise ValueError('Bad URL scheme: {!r}'.format(url))

        if headers is None:
            headers = {}

        headers.setdefault(
            'User-Agent', '%s Python-urllib/%s' %
...
class CurlHTTPFetcher(HTTPFetcher):
    ...
    def fetch(self, url, body=None, headers=None):
        ...
        headers.setdefault(
            'User-Agent',
            '{} {}'.format(USER_AGENT, pycurl.version),
        )
        ...
        try:
            ...
            while off > 0:
                if not self._checkURL(url):
                    raise HTTPError(
                        'Fetching URL not allowed: {!r}'.format(url))
            ...
            raise HTTPError('Timed out fetching: {!r}'.format(url))
        finally:
            c.close()
...
class HTTPLib2Fetcher(HTTPFetcher):
    ...
    def fetch(self, url, body=None, headers=None):
        ...
        if not (url.startswith('http://') or url.startswith('https://')):
            raise ValueError('URL is not a HTTP URL: {!r}'.format(url))
