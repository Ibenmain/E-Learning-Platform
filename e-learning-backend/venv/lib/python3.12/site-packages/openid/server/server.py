...
class CheckAuthRequest(OpenIDRequest):
    ...
    def __str__(self):
        if self.invalidate_handle:
            ih = ' invalidate? {!r}'.format(self.invalidate_handle)
        else:
            ih = ''
        s = '<{} handle: {!r} sig: {!r}: signed: {!r}{}>'.format(
...
class DiffieHellmanSHA1ServerSession:
    ...
    def fromMessage(cls, message):
        ...
        if (
            dh_modulus is None and dh_gen is not None or dh_gen is None and
        ...
        if consumer_pubkey is None:
            raise ProtocolError(
                message, 'Public key for DH-SHA1 session '
...
class AssociateRequest(OpenIDRequest):
    ...
    def fromMessage(klass, message, op_endpoint=UNUSED):
        ...
        try:
            ...
            raise ProtocolError(
                message,
                'Unknown session type {!r}'.format(session_type),
            )
        ...
        try:
            ...
            raise ProtocolError(
                message, 'Error parsing %s session: %s' %
    ...
    def answer(self, assoc):
        ...
        if not (
            self.session.session_type == 'no-encryption' and
...
class CheckIDRequest(OpenIDRequest):
    ...
    def fromMessage(klass, message, op_endpoint):
        ...
        self.trust_root = (
            message.getArg(OPENID_NS, trust_root_param) or
            self.return_to
        )
    ...
    def __repr__(self):
        return '<{} id:{!r} im:{} tr:{!r} ah:{!r}>'.format(
...
class Signatory:
    ...
    def createAssociation(self, dumb=True, assoc_type='HMAC-SHA1'):
        ...
        handle = '{{{}}}{{{:x}}}{{{}}}'.format(
            assoc_type, int(time.time()), uniq)

        assoc = Association.fromExpiresIn(
            self.SECRET_LIFETIME, handle, secret,
            assoc_type,
        )
...
class EncodingError(Exception):
    ...
    def __str__(self):
        if self.explanation:
            s = '{}: {}'.format(self.__class__.__name__, self.explanation)
        else:
            s = '{} for Response {}'.format(
...
class UntrustedReturnURL(ProtocolError):
    ...
    def __str__(self):
        return 'return_to {!r} not under trust_root {!r}'.format(
            self.return_to,
            self.trust_root,
        )
