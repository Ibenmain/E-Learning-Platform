...
def getSecretSize(assoc_type):
    if assoc_type == 'HMAC-SHA1':
        ...
        raise ValueError(
            'Unsupported association type: {!r}'.format(assoc_type))
...
class Association:
    ...
    def signMessage(self, message):
        ...
        """
        if (
            message.hasKey(OPENID_NS, 'sig') or
    ...
    def checkMessageSignature(self, message):
        ...
        if not message_sig:
            raise ValueError('{} has no sig.'.format(message))
    ...
    def _makePairs(self, message):
        ...
        if not signed:
            raise ValueError('Message has no signed list: {}'.format(message))
