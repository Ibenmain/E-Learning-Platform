...
from cryptography.x509.general_name import _IPAddressTypes
...
from cryptography.x509.general_name import RegisteredID
...
class DistributionPoint:
    ...
    def __hash__(self) -> int:
        if self.full_name is not None:
            fn: tuple[GeneralName, ...] | None = tuple(
                self.full_name,
            )
        else:
            fn = None
...
class PolicyInformation:
    def __init__(
        ...
        policy_qualifiers: None | (
            typing.Iterable[str | UserNotice]
        ),
    ) -> None:
...
class NameConstraints(ExtensionType):
    ...
    def __hash__(self) -> int:
        if self.permitted_subtrees is not None:
            ps: tuple[GeneralName, ...] | None = tuple(
        ...
        if self.excluded_subtrees is not None:
            es: tuple[GeneralName, ...] | None = tuple(
