...
def _get_canonical_querystring(query):
    ...
    for key in sorted_keys:
        for item in querystring_encoded_map[key]:
            querystring_encoded_pairs.append(f'{key}={item}')
...
def _generate_authentication_header_map(
    ...
    payload_hash = hashlib.sha256(
        (request_payload or '').encode('utf-8')).hexdigest()
...
class _DefaultAwsSecurityCredentialsSupplier(AwsSecurityCredentialsSupplier):
    ...
    def get_aws_security_credentials(self, context, request):
        ...
        env_aws_access_key_id = os.environ.get(
        ...
        env_aws_session_token = os.environ.get(
    ...
    def get_aws_region(self, context, request):
        ...
        if response.status != http_client.OK:
            raise exceptions.RefreshError(
                f'Unable to retrieve AWS region: {response_body}',
            )
    ...
    def _get_metadata_security_credentials(
        ...
        response = request(
            url=f'{self._security_credentials_url}/{role_name}',
            method='GET',
            headers=headers,
        )
        ...
        if response.status != http_client.OK:
            raise exceptions.RefreshError(
                f'Unable to retrieve AWS security credentials: {response_body}',
            )
    ...
    def _get_metadata_role_name(self, request, imdsv2_session_token):
        ...
        if response.status != http_client.OK:
            raise exceptions.RefreshError(
                f'Unable to retrieve AWS role name {response_body}',
            )
...
class Credentials(external_account.Credentials):
    ...
    def _create_default_metrics_options(self):
        metrics_options = super()._create_default_metrics_options()
    ...
    def from_info(cls, info, **kwargs):
        ...
        return super().from_info(info, **kwargs)
    ...
    def from_file(cls, filename, **kwargs):
        ...
        Returns:
            google.auth.aws.Credentials: The constructed credentials.
        """
        return super().from_file(filename, **kwargs)
