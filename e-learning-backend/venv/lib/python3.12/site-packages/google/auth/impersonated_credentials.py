...
def _make_iam_token_request(
    ...
    response = request(url=iam_endpoint, method='POST',
            ...
            for _ in retries:
                ...
                if response.status_code != http_client.OK:
                    raise exceptions.TransportError(
                        f'Error calling sign_bytes: {response.json()}',
                    )
    ...
    def refresh(self, request):
        ...
        if response.status_code != http_client.OK:
            raise exceptions.RefreshError(
                f'Error getting ID token: {response.json()}',
            )
...
def _sign_jwt_request(request, principal, headers, payload, delegates=[]):
    ...
    try:
        ...
        new_exc = exceptions.RefreshError(
            f'{_REFRESH_ERROR}: No signed JWT in response.', response_body,
        )
        raise new_exc from caught_exc
