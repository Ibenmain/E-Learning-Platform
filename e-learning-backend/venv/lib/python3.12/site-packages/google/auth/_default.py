    ...
    with open(filename) as file_obj:
        try:
            ...
            new_exc = exceptions.DefaultCredentialsError(
                f'File {filename} is not a valid json file.', caught_exc,
            )
...
def load_credentials_from_dict(
    ...
    if not isinstance(info, dict):
        raise exceptions.DefaultCredentialsError(
            f'info object was of type {type(info)} but dict type was expected.',
        )
...
def _load_credentials_from_info(
    ...
    if credential_type == _AUTHORIZED_USER_TYPE:
        ...
        credentials, project_id = _get_gdch_service_account_credentials(
            filename, info)
    else:
        raise exceptions.DefaultCredentialsError(
...
def _get_gae_credentials():
    ...
    try:
        _LOGGER.debug(
            'Checking for App Engine runtime as part of auth process...')
...
def _get_external_account_credentials(
    ...
    if info.get('subject_token_type') == _AWS_SUBJECT_TOKEN_TYPE:
        ...
        try:
            ...
            raise exceptions.DefaultCredentialsError(
                f'Failed to load external account credentials from {filename}',
            )
    if request is None:
...
def _get_external_account_authorized_user_credentials(
    ...
    try:
        ...
        credentials = external_account_authorized_user.Credentials.from_info(
            info)
    except ValueError:
        raise exceptions.DefaultCredentialsError(
...
def _get_authorized_user_credentials(filename, info, scopes=None):
    ...
    try:
        ...
        msg = f'Failed to load authorized user credentials from {filename}'
...
def _get_service_account_credentials(filename, info, scopes=None, default_scopes=None):
    ...
    try:
        ...
        msg = f'Failed to load service account credentials from {filename}'
        new_exc = exceptions.DefaultCredentialsError(msg, caught_exc)
        raise new_exc from caught_exc
    ...
    try:
        ...
        if start_index == -1 or end_index == -1 or start_index > end_index:
            raise exceptions.InvalidValue(
                f'Cannot extract target principal from {impersonation_url}',
            )
...
def _get_gdch_service_account_credentials(filename, info):
    ...
    try:
        ...
        msg = f'Failed to load GDCH service account credentials from {filename}'
        new_exc = exceptions.DefaultCredentialsError(msg, caught_exc)
        raise new_exc from caught_exc
...
def default(scopes=None, request=None, quota_project_id=None, default_scopes=None):
    ...
    explicit_project_id = os.environ.get(
        environment_vars.PROJECT, os.environ.get(
    ...
    checkers = (
        ...
        lambda: _get_gce_credentials(
            request, quota_project_id=quota_project_id),
    )
        ...
        if credentials is not None:
            ...
            if not effective_project_id and callable(
                ...
                if request is None:
                    ...
                    request = google.auth.transport.requests.Request()
                effective_project_id = credentials.get_project_id(
                    request=request)
