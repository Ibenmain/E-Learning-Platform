...
class Credentials(external_account.Credentials):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._credential_source_executable = credential_source.get(
            'executable')
    ...
    def retrieve_subject_token(self, request):
        ...
        response = json.loads(result.stdout.decode(
            'utf-8')) if result.stdout else None
        if not response and self._credential_source_executable_output_file is not None:
            response = json.load(
                open(self._credential_source_executable_output_file,
                     encoding='utf-8'),
            )
    ...
    def _validate_revoke_response(self, response):
        ...
        if not response['success']:
            raise exceptions.RefreshError(
                'Revoke failed with unsuccessful response.')
