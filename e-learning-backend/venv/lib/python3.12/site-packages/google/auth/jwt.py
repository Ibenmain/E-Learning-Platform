...
def _decode_jwt_segment(encoded_section):
    ...
    try:
        ...
        new_exc = exceptions.MalformedError(
            f"Can't parse segment: {section_bytes}",
        )
...
def _unverified_decode(token):
    ...
    if token.count(b'.') != 2:
        raise exceptions.MalformedError(
            f'Wrong number of segments in token: {token}',
        )
    ...
    if not isinstance(header, Mapping):
        raise exceptions.MalformedError(
            f'Header segment should be a JSON object: {encoded_header}',
        )

    if not isinstance(payload, Mapping):
        raise exceptions.MalformedError(
            f'Payload segment should be a JSON object: {encoded_payload}',
        )
...
def _verify_iat_and_exp(payload, clock_skew_in_seconds=0):
    ...
    if latest < now:
        raise exceptions.InvalidValue(f'Token expired, {latest} < {now}')
...
def decode(token, certs=None, verify=True, audience=None, clock_skew_in_seconds=0):
    ...
    try:
        ...
        if key_alg in _CRYPTOGRAPHY_BASED_ALGORITHMS:
            ...
            raise exceptions.InvalidValue(
                f'Unsupported signature algorithm {key_alg}',
            ) from exc
    ...
    if isinstance(certs, Mapping):
        if key_id:
            if key_id not in certs:
                raise exceptions.MalformedError(
                    f'Certificate for key id {key_id} not found.',
                )
